...
test("DeepSeek thinking stays enabled for initial and repair requests with per-proposer effort", () => {
  ...
  const config = {
    ...
    reasoningEffort: "low",
    proposerModels: { librarianProposer: { reasoningEffort: "high" } },
  };
  const request = {
  ...
  for (const repairContext of [
    undefined,
    { assistantOutput: '{"invalid":true}', userMessage: "Repair the rejected output" },
  ]) {
...
test("DeepSeek tool argument parser only repairs excess trailing closing braces", () => {
  assert.deepEqual(parseToolArguments('{"ok":true}}'), {
    output: { ok: true },
    recovery: "trimmed_1_trailing_brace",
    error: null,
  });
  assert.deepEqual(parseToolArguments('{"ok":true}}}'), {
    output: { ok: true },
    recovery: "trimmed_2_trailing_brace",
    error: null,
  });
...
test("incomplete repair candidates stay quoted in user diagnostic data", () => {
  ...
  assert.equal(
    messages.some(({ role }) => role === "assistant"),
    false,
  );
            ...
            choices: [
              {
                ...
                message: { tool_calls: [{ function: { name: "probe", arguments: '{"ok":true}' } }] },
              },
            ],
          }),
        };
...
test("structured transport enforces input capability before dispatch", async () => {
  ...
  await assert.rejects(
    () => invoke({ systemPrompt: "long prompt", userPayload: {}, responseSchema: { name: "x", schema: {} } }),
    /exceeds configured context capability/,
  );
  ...
  const invoke = createStructuredTransport(
    ...
    },
    {
      fetchImpl: async () => ({
        ok: false,
        status: 400,
        json: async () => ({ error: { code: "content_filter", message: "blocked by safety policy" } }),
      }),
    },
  );
...
test("OpenCode Go JSON object adapter puts the bound schema in the prompt and parses JSON locally", async () => {
  ...
  assert.deepEqual(
    requests[0].body.messages.map(({ role }) => role),
        ...
        oneOf: [
          {
            type: "object",
            properties: {
              refs: { type: "array", uniqueItems: true, minItems: 2, items: { type: "string", minLength: 1 } },
            },
          },
