...
function addEvent(overrides = {}) {
  const value = {
    id: "todo:1", text: "归还书", sourceRefs: [ref],
  ...
  return {
    ...
    cleanup_type: null,
    normalized_operation: { op: "addItem", value, sourceRefs: [ref] },
    ...overrides,
  };
}
...
function overdueEvent(overrides = {}) {
  return {
    ...
    cleanup_type: "todo_became_overdue",
...
for (const [name, mutate, pattern] of [
  ["schema mismatch", ({ group }) => { group.schema_version = "2.0"; }, /schema version mismatch/],
  ...
  ["unknown accepted operation", ({ events }) => { events[0].op = "futureOp"; events[0].normalized_operation = { op: "futureOp" }; }, /futureOp cannot target section/],
