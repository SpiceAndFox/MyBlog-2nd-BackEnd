...
test("schema inspection cannot report clean when any v2 table, column, or index is missing", () => {
  ...
  assert.equal(evaluateInspection({ ...base, tables: [...base.tables, "chat_rag_chunks"] }).clean, false);
  ...
  assert.equal(evaluateInspection({ ...base, tables: [...base.tables, "chat_context_projection_checkpoints"] }).clean, false);
