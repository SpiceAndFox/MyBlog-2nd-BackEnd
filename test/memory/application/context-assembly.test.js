...
function config() {
  return {
    enabled: true,
    gapBridge: contextScenario.gapBridge,
...
test("effective-view cleanup requests one idempotent housekeeping wake-up", async () => {
  ...
  let wakes = 0;
  const assemble = createMemoryContextAssembly({ repositories: data.repositories, config: config(), recentWindowMaxChars: contextScenario.recentWindowMaxChars, scheduleHousekeeping() { wakes += 1; } });
...
test("unsupported v2.0 authority is not passed to 2.01 state recovery", async () => {
  ...
  let recoveries = 0;
  const assemble = createMemoryContextAssembly({ repositories: data.repositories, config: config(), recentWindowMaxChars: contextScenario.recentWindowMaxChars, scheduleStateRecovery() { recoveries += 1; } });
