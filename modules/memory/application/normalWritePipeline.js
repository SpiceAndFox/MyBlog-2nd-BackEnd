...
function createNormalWritePipeline({ observer, providerAdapter, repositories, config, now = () => new Date(), idFactory = () => crypto.randomUUID() } = {}) {
  ...
  async function recordAdapterError(envelope, adapterResult) {
    return repositories.withTransaction(async (client) => {
      ...
      const halted = !retryable || consecutiveErrors >= haltAfter;
  ...
  async function handleUnableToDecide(envelope, output) {
    return repositories.withTransaction(async (client) => {
      ...
      if (expansionAttempt === 0) {
        await repositories.runtime.updateTask(envelope.task.taskId, {
          status: "queued", stage: "context_expansion", stage_payload: { persistedProposal: output },
      ...
      await repositories.audit.insertEventGroup({
        ...
        schema_version: SCHEMA_VERSION, base_revision: state.meta.revision, result_revision: nextState.meta.revision,
  ...
  return Object.freeze({ processScope, processIntent, processEnvelope, createTask, createSuccessor, commit, commitWithRecovery, recordAdapterError });
}
