...
standingAgreements 记录**双方或一方明确建立、修改或取消的持续行为规则、沟通规则、边界或反复适用的互动约定**。

进入标准——必须同时满足：
- 是持续适用的规则或约定（非一次性）
- 双方或一方明确建立、修改或取消
- 是可执行的互动行为规则，而非抽象的情感表达
...
- "我永远不会离开你" → 情感表达/关系宣誓，通常归 relationship 或 noop
- "我会一直爱你" → 情感表达，不是行为规则
- "我们是最重要的人" → 关系状态描述，归 relationship
- "明天先给我发消息" → 一次性 todo，归 todos
- "我不喜欢被连续追问" → 用户偏好，归 userProfile
- "我们已经开始互相信任" → 关系转折，归 relationship/milestones
- "帮我拿一下杯子" → 即时一次性指令，通常不进入任何 Memory section。只有它明确形成后续 pending 事项时，才由 todoProposer 处理。

核心区分原则：问自己"这条内容是否为未来的反复互动建立了明确的规则？"如果答案是"是"，它是 agreement；如果答案是"这只是表达感受/描述关系/一次性的请求"，它应该归其他 section。
...
3. 按 §2 的准入标准判断是否属于 standingAgreements——如果更像 todo / profile / relationship / milestone 则 noop。
...
### ❌ 一次性请求当 agreement
用户消息："帮我拿一下杯子"
→ 这是即时一次性指令，不属于持续互动约定。通常所有 Memory section 都应 noop；只有它明确形成后续 pending 事项时才由 todoProposer 处理。

### ❌ 情感表达当 agreement
用户消息："我永远不会离开你"
→ 这是情感表达/关系宣誓，不是可执行的互动规则。通常应归 relationship 或 noop。

### ❌ 用户偏好当 agreement
用户消息："我不喜欢被连续追问"
→ 这是用户偏好/边界，应归 userProfile，不是持续互动约定

### ❌ 关系状态当 agreement
用户消息："我们已经开始互相信任"
→ 这是关系状态/转折，应归 relationship 或 milestones
