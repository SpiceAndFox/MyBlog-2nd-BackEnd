...
- `writableState.working.todos`：当前 todos 的权威基线。每个 item 包含 `id`、`text`、`actor`、`requester`、`status`（active/overdue）、`dueAt` 等。只对确实发生变化的 item 提出 patch；不要对与基线语义相同的内容重复 add。
...
- 模糊愿望（"真希望以后一直开心"）
- 持续互动约定（"以后沉默时先开口"→ standingAgreements）
...
- 消息中出现完整年月日 → `{ "mode": "absolute", "date": "YYYY-MM-DD" }`
- 消息中出现相对时长 → `{ "mode": "relative", "days": N }` 或 `{ "months": N }` 或 `{ "years": N }`
- "明天/两周后/下个月" 等使用 relative 表达式，不得自行换算成 absolute 日期
- "十号/下周末" 等无法唯一确定日期时，不输出 dueAt；不要自行猜年月日
...
### ✅ addItem + user_commitment + absolute dueAt（明确日期）
用户消息 133："我们十号去玩吧"（上下文可确定月份）
...
### ❌ 持续互动约定当 todo
用户消息："以后沉默的时候先说一声"
→ 这是持续互动约定，应归 standingAgreements，不是 todos → noop
...
12. 是否对 writableState 中已存在的语义相同 todo 重复 addItem？—如果是，去掉。
