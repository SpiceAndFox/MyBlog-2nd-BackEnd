...
13. 对于 relative dueAt：是否恰好只含一个单位，且 `days >= 0`、`months >= 1`、`years >= 1`？今天必须规范表示为 `{ "mode": "relative", "days": 0 }`，不得同时输出其他零值单位。
14. 是否对 writableState 中已存在的语义相同 todo 重复 addItem？—如果是，去掉。
15. 重新安排 overdue todo 时是否使用 updateItem + dueChange.mode=set，而非 addItem？
16. 若目标旧 todo 不在 writableState，是否输出 unable_to_decide，而非猜测 itemId？
