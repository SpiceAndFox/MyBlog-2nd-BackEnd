...
写入语义最匹配的单个 section，不机械复制。个人偏好/边界属于对应 Profile；未来反复适用的行为约定不是 relationship 描述，本 target 对其 noop；relationship 只保存双方已经成立的关系事实或互动结构。
...
- facet 表示事实类别；canonicalKey 表示会影响未来回应的稳定槽位。选择最具体的 key，只有没有合适槽位才用 `open`；同一非多值 key 已存在时 update/noop，不换 key 重复 add。
- 每个 patch 使用非空 `evidenceRefs`；`messageId` 来自 `observedMessages`，`quote` 是直接支持事实的最短连续原文，不改写、不拼接，归一化后至少 3 个信息字符，最多 200 Unicode code points。
- observedPattern 至少 3 个不同 messageId，并满足独立片段与主体约束。

## 判断示例

- 用户直接声明长期沟通边界 → userProfile，`factBasis=explicit`。
- “不要连续追问我” → userProfile：`facet=communicationBoundary, canonicalKey=followUpQuestions`。
- “用户称 assistant 为小月” → relationship：`facet=address, canonicalKey=userToAssistantAddress`。
