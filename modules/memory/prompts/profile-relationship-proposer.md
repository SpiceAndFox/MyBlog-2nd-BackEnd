...
- updateItem 只能用 user_correction 或 assistant_correction。
  ...
  - 不得为了减少 item 数量而把不同属性、偏好或边界塞入已有 item；容量合并只由 compactionProposer 处理。
...
13. 是否把独立新事实错误地 updateItem 到了已有 item？—如果是，改为 addItem。
14. forgetItem 只在消息明确指向 writableState 中已存在的具体 item 时输出。
15. 是否对 writableState 中已存在的语义相同 item 重复 addItem？—如果是，去掉。
