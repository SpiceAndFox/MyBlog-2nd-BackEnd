...
- updateItem 只能用 user_correction 或 assistant_correction——正常补充也用此路径。
...
- 行为推断仅在窗口内观察到明确、显著的模式时才输出，且要求至少 2 条独立的 evidenceRefs。
- 一次性动作不构成 trait（一次沉默不推断"回避型人格"）。
- 不得把多个 tick 中模糊重复的行为自行累计成稳定人格——只基于本次 observedMessages 窗口内的证据。
- User 与 Assistant 的发言均可支持三个 section 的新增，不因消息 role 限制操作哪个 profile。
...
- addItem 至少 1 条 evidenceRef；行为推断类 addItem 建议至少 2 条。
...
→ userProfile.addItem，text="偏好: 回避直接冲突"，evidenceKind=long_term_fact，含至少 2 条 evidenceRefs

### ✅ userProfile.updateItem + user_correction
writableState 中 userProfile:1 为"偏好: 不喜欢被连续追问"，用户消息 170："我不喜欢别人突然碰我"
→ userProfile.updateItem，itemId="userProfile:1"，value.text="偏好: 不喜欢被连续追问 | 讨厌突然肢体接触"，evidenceKind=user_correction
...
### ❌ 把情感表达当 standing agreement
用户消息："我永远不会离开你"
→ 这是情感表达/关系宣誓，不是可执行的互动规则。通常应归 relationship（如果实质改变了关系状态），或 noop（如果只是情绪流露）。不应归 userProfile。
...
12. 行为推断类 addItem 至少 2 条 evidenceRefs。
13. forgetItem 只在消息明确指向 writableState 中已存在的具体 item 时输出。
14. 是否对 writableState 中已存在的语义相同 item 重复 addItem？—如果是，去掉。
