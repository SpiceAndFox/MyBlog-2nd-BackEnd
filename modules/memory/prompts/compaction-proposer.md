...
- 两条内容有否定差异（"不喜欢突然触碰" vs "接受拥抱"）→ 不能合并，语义方向相反。
- 两条相互冲突的事实 → 不能通过合并"调和"。
...
- 正例（合并两条 userProfile）：source items "偏好: 晚上聊天" + "关系模式: 慢热后依赖" → `"偏好/关系模式: 夜间更适合长聊 | 慢热后依赖"`
- 正例（合并两条 worldFacts）：source items "魔法规则: 月光下生效" + "魔法规则: 月圆时威力翻倍" → 不能合并（内容不同，不重叠）→ 应 unable_to_compact
- 反例：source items "偏好: 喜欢安静" + "偏好: 不喜欢被打断" → 如果合并成"性格: 内向"，则引人了 source items 未表达的新推断 → 非法
...
### ✅ mergeItems（合并重复 userProfile）
writableState 中 userProfile:1 为"偏好: 晚上聊天"，userProfile:2 为"关系模式: 需要慢慢熟悉后再依赖"
→ mergeItems，itemIds=["userProfile:1", "userProfile:2"]，value.text="偏好/关系模式: 夜间更适合长聊 | 慢热后依赖"
...
→ 这两条可能不冲突（拥抱可能不是"突然触碰"），但如果语义方向相反则需要非常谨慎。若无法确定它们是否真正兼容 → unable_to_compact
