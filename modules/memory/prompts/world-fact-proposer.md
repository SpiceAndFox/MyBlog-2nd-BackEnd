...
- `noop`：已理解 new batch，并确认 worldFacts 无需变化。
...
- 叙事者/角色以确定语气建立新设定，可以写入；assistant 对用户设定作出的推断、补全或装饰性扩写，不写入。
...
- 与 writableState 已有 worldFacts 冲突但没有 correction 语义时，不得直接 add 一个冲突事实；应 noop 或等待明确的 correction。
- 反例：用户只说"这里有精灵"，assistant 自行补充"精灵不能碰铁器"——这是未经确认的即兴扩写，不应自动写入 worldFacts。
...
3. 按 §2 的准入标准判断是否属于 worldFacts；不满足准入标准则本 section 输出 noop。
...
- 即使原话包含成人或敏感内容，也不得净化、替换或改写 quote；`value.text` 只客观记录设定，不写感官细节。
...
### ❌ Assistant 对用户设定的即兴扩写
用户消息："这里有精灵"，assistant 消息："精灵不能碰铁器，碰到会灼伤"
→ "精灵不能碰铁器"是 assistant 对 user 设定的装饰性扩写，未经 user 明确确认 → 不应自动写入 worldFacts。与上面 ✅ 的区别：那里 assistant 是以确定语气独立设定世界规则，这里是对 user 已有设定的即兴补全/装饰，需要 user 明确确认或 assistant 以独立设定语气陈述才写入。
...
### ❌ 场景暂时状况当 worldFacts
用户消息："今晚魔法失效了"
→ 这是暂时情况，不是持续世界设定；`worldFacts` 输出 noop
...
### ❌ 角色档案当 worldFacts
用户消息："她害怕魔法"
→ 这是单个角色的属性，不是世界设定；`worldFacts` 输出 noop

### ❌ 持续约定当 worldFacts
用户消息："我们决定不用魔法伤害彼此"
→ 这是双方互动规则，不是世界设定；`worldFacts` 输出 noop
...
12. Assistant 的 addItem 是否为以确定语气独立设定的世界规则，而非对 user 设定的即兴补全/装饰性扩写？—如果是后者，去掉。
13. 与 writableState 已有 worldFacts 冲突但没有 correction 语义时，是否直接 add 了一个冲突事实？—如果是，改为 noop。
14. 是否对 writableState 中已存在的语义相同 worldFact 重复 addItem？—如果是，去掉。
