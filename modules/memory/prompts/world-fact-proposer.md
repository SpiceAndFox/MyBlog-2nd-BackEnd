...
```json
{"tickId":0,"proposer":"worldFactProposer","sectionResults":{"worldFacts":{"status":"changes","changes":[{"action":"add","text":"魔法仅在月光直射时生效","evidenceMessageIds":[101]}]}}}
```
...
1. 核心断言描述世界本身的规则、结构、环境与设定——物理/自然/技术/魔法规则、地理与环境、制度与社会结构、种族或群体层面的通用设定；它不属于世界中某个实体（用户或 Assistant）的个人属性、身份、偏好或经历，也不属于实体之间的关系；或者仍会影响当前 canon 的历史事实。归属按核心语义判断，不按句子的语法主语或消息 role 判断。
