const { SCHEMA_VERSION, TARGET_KEYS, SCENE_FIELDS } = require("./constants");
...
function sourceKey(ref) { return `${ref.messageId}\u0000${ref.contentHash}`; }

function validateSourceRefs(refs, path, errors, { allowEmpty = false } = {}) {
  ...
  refs.forEach((ref, index) => {
    ...
    if (!exactObject(ref, ["messageId", "contentHash"], refPath, errors)) return;
  ...
  if (errors.some((entry) => entry.path.startsWith(path))) return;
  const sorted = refs.slice().sort((left, right) => left.messageId - right.messageId || left.contentHash.localeCompare(right.contentHash));
  if (new Set(refs.map(sourceKey)).size !== refs.length || sorted.some((ref, index) => sourceKey(ref) !== sourceKey(refs[index]))) {
...
function validateSceneField(field, path, errors) {
  ...
  if (field.value === null) {
    validateSourceRefs(field.sourceRefs, `${path}.sourceRefs`, errors, { allowEmpty: true });
    if (field.sourceRefs?.length || field.updatedAtMessageId !== null) add(errors, path, "empty field must clear provenance");
    return;
  }
  validateSourceRefs(field.sourceRefs, `${path}.sourceRefs`, errors);
...
function validateMemoryState(value) {
  ...
  if (value.version !== SCHEMA_VERSION) add(errors, "$.version", `must equal ${SCHEMA_VERSION}`);
  if (exactObject(value.current, ["scene", "previousScene"], "$.current", errors)) {
    if (exactObject(value.current.scene, SCENE_FIELDS, "$.current.scene", errors)) {
      for (const field of SCENE_FIELDS) validateSceneField(value.current.scene[field], `$.current.scene.${field}`, errors);
    }
    if (value.current.previousScene !== null) {
      if (exactObject(value.current.previousScene, [...SCENE_FIELDS, "expiredAt"], "$.current.previousScene", errors)) {
        for (const field of SCENE_FIELDS) validateSceneField(value.current.previousScene[field], `$.current.previousScene.${field}`, errors);
  ...
  for (const [container, sections, path] of containers) {
    ...
    for (const section of sections) {
      ...
      container[section].forEach((item, index) => {
        validateItem(item, section, `${path}.${section}[${index}]`, errors);
...
function assertMemoryState(value) {
  const validation = validateMemoryState(value);
  if (!validation.ok) {
