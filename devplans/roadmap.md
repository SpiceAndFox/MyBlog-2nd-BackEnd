...
3. **Normal 写入链路**：实现 envelope/schema、六个 Proposer prompt、Memory Provider Adapter、Observer、窗口/eligibility、cursor、durable task 与原子提交；先 mock Adapter，后可使用本机 `.env` 的 DeepSeek v4 flash 做 prompt golden/smoke。
