...
Memory v2 的长期 item（milestones、core）携带 `evidenceGroups`，其中每个 ref 的 `quote` 是 <=80 字符的短片段。对长期 item 而言，quote 脱离场景后上下文价值很薄——"我愿意相信你"这五个字不比 text "关系转折: 第一次明确互相信任" 多传递什么信息。
