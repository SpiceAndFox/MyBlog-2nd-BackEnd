...
RAG/Recall 查询截止点：主聊天查询时，RAG/Recall 只检索 `messageId < recentWindowStartMessageId` 的消息（recent window 已完整覆盖这些消息），同时不超过各自 projection 的 `processedBoundaryMessageId`。即有效检索上界为 `min(projection.processedBoundaryMessageId, recentWindowStartMessageId - 1)`。六个 Memory target cursor 与 RAG/Recall cutoff 相互独立：Memory cursor 追平不代表 RAG/Recall 追平，反之亦然。

Projection 落后但未影响本次查询时不告警：只有当 projection 未追平的范围与本次查询所需范围（`messageId < recentWindowStartMessageId`）有重叠时，才触发 `degraded/rebuilding` 告警。例如 RAG projection 落后 2 条消息，但 recent window 覆盖了这 2 条，则本次查询不受影响，不告警。
