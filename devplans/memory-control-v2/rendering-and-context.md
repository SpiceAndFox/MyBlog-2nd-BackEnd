...
- active 与 overdue todo 来自同一个 `working.todos` section；Renderer 按 `status` 分组。overdue 组按 `becameOverdueAt DESC` 取配置的最新 N 条，并受独立字符预算约束。
- `current.previousScene` 使用与 scene 相同的四字段快照（额外含 `expiredAt`），仅在标题 `[上次已知场景]` 下渲染，必须明确它不是当前场景。
