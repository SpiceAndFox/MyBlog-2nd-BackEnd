...
[待办]
{working.todos.filter(t => t.status !== "overdue").map(t => `- ${t.text}`).join("\n") || "(无)"}

[已逾期待办]
{working.todos.filter(t => t.status === "overdue").sort(byBecameOverdueAtDesc).slice(0, config.overdueMaxRenderedItems).map(t => `- ${t.text}`).join("\n") || "(无)"}
...
- `recentEpisodes` 的渲染条数是第 6 批随滑动窗口一起落地的确定性例外；本批不保留硬编码“最近 3 条”，最终值从集中配置读取。
- active 与 overdue todo 来自同一个 `working.todos` section，Renderer 按 `status` 分组。overdue 组按 `becameOverdueAt DESC` 取配置的最新 N 条并受独立字符预算约束——该渲染限制是第 6 批随 todo overdue 生命周期一起落地的确定性例外。
- `current.previousScene` 使用与 scene 相同的四字段快照（额外含 `expiredAt`），仅在标题 `[上次已知场景]` 下渲染，必须明确它不是当前场景——该字段的写入与渲染是第 6 批随 scene 过期生命周期一起落地的确定性例外。
