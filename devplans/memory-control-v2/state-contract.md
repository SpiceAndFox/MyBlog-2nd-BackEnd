...
`evidenceGroups` 是权威 state 中 item 的证据结构。一个 group 是一个可审计、可 recall 的证据单元；group 内多个 ref 共同支撑该单元。普通非 `mergeItems` patch 仍输出 `evidenceRefs`，Reducer 校验通过后将该数组包装成一个新的 `evidenceGroup`。`mergeItems` 不接收 Proposer 输出的 `evidenceRefs`，Reducer 从 source items 继承 `evidenceGroups`，保留 group 边界。`updatedAtMessageId` 是全部 `evidenceGroups.refs.messageId` 的最大值。

`scene` 是当前状态，每个字段独立记录值、证据与更新时间。`todos`、`standingAgreements` 与 `recentEpisodes` 是工作区记忆；`milestones` 与 core 各数组位于长期区并保留 item 级证据。
...
- `itemIds`（数组）：对 `mergeItems` 必填，指定要合并的多个 itemId。`value` 是合并后的新 item 值，至少包含 `text`。merged item 的 `evidenceGroups` 由 Reducer 从 source items 自动继承，保留 group 边界。
- `value`：对 `setField`/`addItem`/`updateItem` 必填。
- `evidenceRefs`：除 `mergeItems` 和 Reducer 自行触发的过期清理外，patch 至少包含一个 `{ messageId, quote }`。普通写入 patch 的 `evidenceRefs` 必须来自 Proposer envelope 的 `observedMessages`（见 §5）。对会写入 item 的普通 patch，Reducer 将该数组包装成一个新的 `evidenceGroup`。
...
Proposer 输入中的 state item 使用 redacted view，不包含 `evidenceGroups`。item 形态：
...
- `writableState`：本次允许写入的目标 section/path 当前状态。item 使用 §5 的 redacted view；无值字段显式传 null。
- `readOnlyContext`：可读取的背景 memory，用于理解对话，不得作为新事实证据。item 使用 §5 的 redacted view，固定范围见 §5.3。
...
- quote 校验材料由 Reducer 从 `observedMessages` 或数据库读取，不作为 Proposer envelope 字段暴露。
...
- `writableState` 与 `readOnlyContext` 不暴露既有 `evidenceGroups` 或 quote。
...
- 固定范围以完整 segment 为单位：一个 section/path 一旦纳入，全量输入该 segment 当前 items 的 redacted view，无值字段显式传 null，不做 last N、相似度筛选或关键词筛选。
...
- `patches` 数组中每个 patch 含 `op`、`path`/`itemId`/`itemIds`（按 §4 必填规则）、`value`（按 §4 必填规则）、`evidenceKind`（§3 枚举）。除 `mergeItems` 外，每个 patch 还必须含 `evidenceRefs`（至少 1 项，每项含 `messageId` integer 和 `quote` string max 80 字符）。
- `evidenceRefs` 是 Proposer patch 字段，不是 item 的权威存储字段。Reducer 对普通非 `mergeItems` patch 校验通过后，将该数组包装为一个 `evidenceGroup` 写入 item；`mergeItems` 的 evidenceGroups 从 source items 继承。
...
- `compactionProposer` 的 schema 额外限制：只能输出 `mergeItems`，且 `evidenceKind` 只能是 `memory_compaction`。输出 `addItem`、通用删除、跨 section 合并、跨 core path 合并、`user_correction` 或 `evidenceRefs` 均非法。
- `sectionResults` 必须恰好覆盖 `task.targetSections`。缺少 target section、包含非 target section、目标 section 缺少 `status`，均由 adapter/tick orchestrator 归类为 `output_schema_invalid`，不交 Reducer，不推进 cursor，并按 §9.2 触发 halt。
...
Reducer 校验普通非 `mergeItems` patch 的 `evidenceRefs.quote` 是否能在对应 `messageId` 的 raw message 内容中找到。普通模式下，patch evidence 必须来自 `observedMessages`。校验通过后，Reducer 将该 patch 的 `evidenceRefs` 包装为一个 `evidenceGroup`；`createdAtMessageId` 取首个写入 group 的最小 messageId，`updatedAtMessageId` 取全部 groups 内全部 refs 的最大 messageId。

`mergeItems` 不接收 Proposer 输出的 `evidenceRefs`，Reducer 校验 source items 存在且带有结构合法的 `evidenceGroups`，然后继承到 merged item。source evidence 已在写入 source item 时通过 quote 校验。
...
| `standingAgreements`        | 12            | 暂缓新增，触发 compaction task；失败后最终拒绝 |
...
CREATE TABLE chat_memory_ops_log (
  ...
  outcome         TEXT NOT NULL,           -- llm_call_failed | safety_policy_blocked | output_schema_invalid | unable_to_decide | halted
...
- `halted`：用于瞬时错误（`llm_call_failed`/`safety_policy_blocked`）达 `consecutiveErrors` 阈值（3 次）后的 halt 转移。`unable_to_decide` 路径不使用此 outcome。halt 作用于该 `userId/presetId` 的会话——`meta.halted` 存在于单个 `memory_state`，true 时该会话的聊天接口拒绝新消息，不影响其他会话。

ops_log 只记 patch 产生之前的非正常结果和 halt。patch 是否被 Reducer 接受、拒绝、暂缓，查 events 表。
...
- `status: "ok"` → tick orchestrator 检查 output 的 `sectionResults`：缺少 target section、包含非 target section 或目标 section 缺少 `status` 时，写 ops_log（outcome=`output_schema_invalid`）并触发 halt，不交 Reducer；`unable_to_decide` 的 section 由 tick orchestrator 直接处理（写 ops_log + 更新 `meta.recovery`，按 [write-protocol.md](write-protocol.md) §3.1 恢复策略），**不交 Reducer**；`patches`/`noop` 的 section 才交 Reducer 处理（patch 决策落 events 表）。Reducer 永远不接触 `unable_to_decide`。
