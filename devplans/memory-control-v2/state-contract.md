...
| `milestones.updateItem` / `correctItem`             | `user_correction`, `assistant_correction`                                                        | 里程碑保守更新                  |
...
| `core.updateItem` / `correctItem`                   | `user_correction`, `assistant_correction`                                                        | core 只能被明确修正改变         |
...
section 级 cursor 推进是派生的：该 section 该 tick 有任一 `accepted`/`noop` 行即推进；有 `deferred` 行且无 `accepted` 行则不推进——即使存在 `rejected` 行，`deferred` 仍优先阻止推进（见 [write-protocol.md](write-protocol.md) §3）。
