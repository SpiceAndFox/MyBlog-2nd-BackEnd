...
- 如果 adapter 返回 `ok` 但 `sectionResults` 缺了某个 target section（schema 约束未拦住的漏输出），Reducer 视为该 section `rejected`（reason=`schema_invalid`），不猜测推进，落 events 表。
...
| `recentEpisodes.mergeItems`                         | `recent_episode`, `user_correction`, `memory_compaction`                                          | 溢出由滑动窗口处理；`memory_compaction` 为保留扩展（compaction 暂不针对本 section），normal proposer 去重用 `recent_episode`/`user_correction` |
...
| `core.addItem`                                      | `long_term_fact`                                                                                 | 单次临时剧情不得进入            |
...
Memory v2 把"patch 决策"与"LLM 运营事件"分两类记录。前者是 Proposer 产出判断后 Reducer 的写入决策，后者是 Proposer 调用本身的技术性结果（成功、失败、信息不足）。两者查询模式不同：查 memory 演化走 events 表，查 provider 健康度走 ops_log，查待回填 backlog 走 skipped 表。
...
section 级 cursor 推进是派生的：该 section 该 tick 有任一 `accepted`/`noop`/`rejected` 行即推进；有 `deferred` 行且无 `accepted` 行则不推进（见 [write-protocol.md](write-protocol.md) §3）。
...
`item_id` 列：对单 item 操作（`updateItem`/`completeTodo`/`cancelTodo`/`expireTodo`/`correctItem`）存目标 item id；对 `mergeItems` 存 `itemIds.join(",")`；对 `addItem`/`setField`/`clearField`/`noop` 存 null。完整信息在 `patch_summary` JSONB 中。
...
CREATE TABLE chat_memory_ops_log (
  ...
  outcome         TEXT NOT NULL,           -- ok | llm_call_failed | safety_policy_blocked | output_schema_invalid | unable_to_decide | max_retry_exceeded
...
- `unable_to_decide`：Proposer 自认信息不足。首次记录并扩窗口重试一次；二次仍 `unable_to_decide` → 推进 cursor 并写 skipped（reason=`unable_to_decide`）。ops_log outcome 始终记 `unable_to_decide`（记实际发生的事），不升级为 `max_retry_exceeded`。
- `max_retry_exceeded`：仅用于瞬时错误（`llm_call_failed`/`safety_policy_blocked`）达 `consecutiveErrors` 阈值后的最终推进。`unable_to_decide` 路径与 `output_schema_invalid` 直接推进路径均不使用此 outcome。
...
- `consecutiveErrors`：仅瞬时 error outcome（`llm_call_failed`/`safety_policy_blocked`）时 +1，`accepted`/`rejected`/`noop`/`deferred` 时重置为 0。`output_schema_invalid` 不走此计数器——它是持续性错误，直接推进（见 [write-protocol.md](write-protocol.md) §3.1）。达 [write-protocol.md](write-protocol.md) §3.1 阈值（3 次）后强制推进 cursor、写 skipped 表（reason 取最后一次 error outcome：`safety_policy_blocked` 保留本名，`llm_call_failed` 归为 `max_retry_exceeded`）、清零。
- `awaitingContextExpansion`：首次 `unable_to_decide` 置 true、不推进 cursor。下一 tick Observer 读到 true → 发扩大的 contextWindow（如 +10）。二次仍 `unable_to_decide` → 推进 cursor、写 skipped 表（reason=`unable_to_decide`）、清 false。任何 patch 决策（accepted/rejected/noop/deferred）也清 false。
- 两者独立：一个管 error 路径，一个管 unable_to_decide 路径，不互相干扰。

### 9.3 待回填消息表

```sql
CREATE TABLE chat_memory_skipped_messages (
  id              BIGSERIAL PRIMARY KEY,
  user_id         BIGINT NOT NULL,
  preset_id       TEXT NOT NULL,
  section         TEXT NOT NULL,
  message_id      BIGINT NOT NULL,
  skipped_at_tick_id BIGINT NOT NULL,
  reason          TEXT NOT NULL,           -- unable_to_decide | max_retry_exceeded | safety_policy_blocked
  resolved        BOOLEAN NOT NULL DEFAULT FALSE,
  resolved_at_tick_id BIGINT,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (user_id, preset_id, section, message_id)
);

CREATE INDEX idx_skipped_unresolved
  ON chat_memory_skipped_messages(user_id, preset_id, section)
  WHERE resolved = FALSE;
```

当 section 因 `unable_to_decide`/`max_retry_exceeded`/`safety_policy_blocked` 被迫推进 cursor 时，本次 observed 窗口内**未被任何 accepted patch 覆盖**的 message 写入此表，作为回填 backlog。skip 行只在重试耗尽、cursor 确定推进后写入——扩窗口重试时作为重叠上下文被重新看的 message 不算 skipped。

这是 memory 系统自管的 backlog，不污染 message 表：message 仍是干净的原始信息层，skip 是 per-section 粒度（同一条 message 可能对 `core` 被 skip、对 `scene` 正常 accepted），只有在 memory 系统内表达才完整。回填工具按 `WHERE resolved=FALSE AND section=?` 查询；未来重跑这些 message 并成功产生 accepted patch 后置 `resolved=TRUE`，不删行（保留审计）。
...
- `status: "error"` → 不交给 Reducer，直接写 ops_log（outcome=对应 reason）、更新 `meta.recovery`、按 [write-protocol.md](write-protocol.md) §3.1 恢复策略决定是否重试或推进 cursor。若推进 cursor 且原因属于 skipped 范畴，写 skipped 表。
