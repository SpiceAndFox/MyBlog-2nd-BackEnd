...
- `itemIds`（数组）：对 `mergeItems` 必填，指定要合并的多个 itemId。`value` 是合并后的新 item 值，至少包含 `text`。merged item 的 `evidenceRefs` 由 Reducer 从 source items 自动继承并去重。
- `value`：对 `setField`/`addItem`/`updateItem` 必填。
- `evidenceRefs`：除 `mergeItems` 和 Reducer 自行触发的过期清理外，patch 至少包含一个 `{ messageId, quote }`。普通写入 patch 的 `evidenceRefs` 必须来自 Proposer envelope 的 `observedMessages`（见 §5）。
- `scene.setField`/`scene.clearField` 的 `evidenceRefs` 必须恰好 1 条。Reducer 将其写入目标字段的 `evidenceRef`。
- `quote`：必须是短片段（<=80 字符），不保存大段原文。
- `evidenceKind: "memory_compaction"` 只允许用于 `mergeItems`。Proposer 不输出 `evidenceRefs`；Reducer 根据 `itemIds` 从权威 state 读取 source items 的既有 `evidenceRefs` 并写入 merged item。
...
Proposer 输入中的 state item 使用 redacted view，不包含 `evidenceRefs` 或 `quote`。item 形态：
...
| `task.trigger`                        | `{ type: "lengthBudget", limit, blockedPatchSummary }` | `blockedPatchSummary` 只解释触发原因                               |
| `writableState`                       | 目标 section/path 的既有 items 全集                    | item 使用 §5 redacted view；不按 `blockedPatchSummary` 语义筛选    |
| `readOnlyContext`                     | `{}`                                                   | 维护任务只在目标 source items 内判断合并                           |
| `observedMessages`                    | `[]`                                                   | 维护任务不读取 raw messages                                        |

维护模式下，`writableState` 不包含 `evidenceRefs` / `quote`。`compactionProposer` 只输出可合并的 `itemIds` 与合并后的 `value.text`。Reducer 根据 `itemIds` 从权威 `memory_state` 读取 source items，继承并去重所有 source `evidenceRefs`。
...
- `writableState` 与 `readOnlyContext` 不暴露既有 `evidenceRefs` 或 quote。

**证据（维护模式）**

- `task.trigger` 为 `{ type: "lengthBudget", limit, blockedPatchSummary }`。维护 patch 不输出 `evidenceRefs`；Reducer 从 source items 自动继承。
- `blockedPatchSummary` 只解释触发原因。

**readOnlyContext 组装**

- `readOnlyContext` 必须带结构化 section 名称，不能把 Renderer 文本或旧 summary 整段塞给 Proposer。
- 固定范围以完整 segment 为单位：一个 section/path 一旦纳入，全量输入该 segment 当前 items 的 redacted view，无值字段显式传 null，不做 last N、相似度筛选或按 `blockedPatchSummary` 语义筛选。
...
Reducer 校验 `evidenceRefs.quote` 是否能在对应 `messageId` 的 raw message 内容中找到。普通模式下，非 `mergeItems` patch 的 evidence 必须来自 `observedMessages`。`mergeItems` 的 evidence 由 Reducer 从 source items 继承；需要校验时，Reducer 按 source `evidenceRefs.messageId` 读取 raw messages。LLM 经常改写 quote，精确匹配会大量误判，因此采用模糊匹配：
