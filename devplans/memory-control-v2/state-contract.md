...
`evidenceGroups` 是权威 state 中 item 的证据结构。一个 group 是一个可审计、可 recall 的证据单元；group 内多个 ref 共同支撑该单元。普通非 `mergeItems` patch 输出 `evidenceRefs`，Reducer 校验通过后将该数组包装成一个新的 `evidenceGroup` 追加到 item。`mergeItems` 不接收 Proposer 输出的 `evidenceRefs`，Reducer 从 source items 继承 `evidenceGroups`，保留 group 边界。

item 派生字段由 Reducer 在 apply 时维护：`createdAtMessageId` 取首个写入 group 的最小 messageId（addItem 设定，updateItem 不改；mergeItems 取 source items 中最早的 `createdAtMessageId`）；`updatedAtMessageId` 取全部 `evidenceGroups.refs.messageId` 的最大值；`evidenceKind` 取本次 apply 的 patch.evidenceKind（addItem 设定，updateItem/mergeItems 覆盖）。
...
Reducer 校验普通非 `mergeItems` patch 的 `evidenceRefs.quote` 是否能在对应 `messageId` 的 raw message 内容中找到。普通模式下，patch evidence 必须来自 `observedMessages`。校验通过后，Reducer 将该 patch 的 `evidenceRefs` 包装为一个新的 `evidenceGroup` 追加到 item；item 派生字段（`createdAtMessageId`/`updatedAtMessageId`/`evidenceKind`）的维护规则见 §1。
...
- `halted`：瞬时错误（`llm_call_failed`/`safety_policy_blocked`）的 `consecutiveErrors` 达阈值（3 次）时，第 3 次错误的本 tick ops_log 行 outcome 记为 `halted`（不再重复记 `llm_call_failed`/`safety_policy_blocked`），`detail` 仍携带底层错误原因，同步设置 `meta.halted=true`。`unable_to_decide` 路径不使用此 outcome。halt 作用于该 `userId/presetId` 的会话——`meta.halted` 存在于单个 `memory_state`，true 时该会话的聊天接口拒绝新消息，不影响其他会话。
...
- `consecutiveErrors`：仅瞬时错误（adapter 返回 `llm_call_failed`/`safety_policy_blocked`）发生时 +1，与 ops_log 行的 outcome 标签解耦；`accepted`/`rejected`/`noop`/`deferred` 时重置为 0。`output_schema_invalid` 不走此计数器——持续性错误，直接触发 halt（见 [write-protocol.md](write-protocol.md) §3.1）。达 [write-protocol.md](write-protocol.md) §3.1 阈值（3 次）后触发 halt（`meta.halted=true`），`consecutiveErrors` 保留不清零，等手动 resume 时重置。
