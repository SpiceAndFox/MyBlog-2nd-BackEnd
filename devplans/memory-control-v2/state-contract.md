...
- `accepted`：至少一个 patch 被 apply。
- `rejected`：patches 全被拒，或 compaction 无合并空间后的最终预算拒绝。
- `deferred`：新增被长度预算阻塞，已触发 compaction task。
- `noop`：Proposer 明确判断无变化。占位行，`patch_id`/`op`/`item_id`/`evidence_kind`/`patch_summary` 为 null。
...
- `unable_to_decide`：Proposer 自认信息不足。首次记录并扩窗口重试一次；二次仍 `unable_to_decide` 升级为 `max_retry_exceeded` 并推进 cursor。
- `max_retry_exceeded`：连续 error 达阈值、或 unable_to_decide 扩窗口重试耗尽后的最终推进。
...
- `consecutiveErrors`：error outcome 时 +1，`accepted`/`rejected`/`noop`/`deferred` 时重置为 0。达 [write-protocol.md](write-protocol.md) §3.1 阈值后强制推进 cursor、写 skipped 表（reason 取最后一次 error outcome：`safety_policy_blocked` 保留本名，`llm_call_failed`/`output_schema_invalid` 归为 `max_retry_exceeded`）、清零。
- `awaitingContextExpansion`：首次 `unable_to_decide` 置 true、不推进 cursor。下一 tick Observer 读到 true → 发扩大的 contextWindow（如 +10）。二次仍 `unable_to_decide` → 推进 cursor、写 skipped 表、清 false。任何 patch 决策（accepted/rejected/noop/deferred）也清 false。
...
- `status: "ok"` → 将 `output` 交给 Reducer 处理（patch 决策落 events 表）。
