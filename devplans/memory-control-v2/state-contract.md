...
- `value`：对 `setField`/`addItem`/`updateItem` 必填。普通 item 的 `addItem`/`updateItem` value 至少包含 `text`。Todo 使用更窄的结构：
  ...
  - absolute date 的 deadline 是该日期结束后的首个日界线；relative deadline 以本 patch `evidenceRefs` 中 messageId 最大的 evidence message 的数据库 `createdAt` 为 anchor。禁止使用 task/worker 执行时间作 anchor。Reducer 只负责确定性日期计算和 ISO 8601 格式化；已到期的结果仍可写入，并由同一事务或随后 housekeeping 原位标记 overdue，不能因历史回放发生在 deadline 之后而拒绝事实。
...
5. 提交事务必须先按 `task_id` 锁定并读取 task，再校验 source generation、target `cursor_before`、当前 revision 与该 stage 的预期执行 revision；普通首次提交要求当前 revision 等于 task 创建时的 `base_revision`。compaction/replay 按 §2.1 的规则使用各阶段开始时捕获的最新 revision，其他 target 的合法 revision 增长本身不使原 proposal stale，但该阶段仍只能从实际最新 revision 创建下一 revision。
6. 每个 task phase 使用稳定的 event group identity；同一 group 内的 `patch_id` 以及非 null `result_revision` 都有唯一约束。capacity-blocked 审计与最终 replay 是两个稳定 phase/group，因此允许同一原 patchId 在两组中分别出现 deferred 与最终 decision。相同 phase/task/patchId 的重复 delivery、进程恢复或提交结果不确定时，先读取既有终态：已提交则返回原结果，未提交才继续；不得产生第二组 events、第二个 state revision、重复 snapshot 或重复 cursor 推进。compaction、proposal replay、system cleanup 与 cursor-only revision 遵守同一原则。
...
7. `task_id` 与 `dedupe_key` 在任务首次创建后保持稳定。相同调度事实的重复 wake-up 必须命中同一 dedupe key 并读取既有 task，不得新建第二行；normal task 的 key 至少绑定 generation、target、`cursor_before` 与 `target_message_id`，maintenance task 还绑定 `parent_task_id` 与阻塞 section，system cleanup task 绑定确定性 cleanup 边界。key 的具体编码由实现集中定义，不进入 Prompt。
