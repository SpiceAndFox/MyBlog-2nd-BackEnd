...
5. 提交事务必须先按 `task_id` 锁定并读取 task，再校验 source generation、target `cursor_before`、当前 revision 与该 stage 的预期执行 revision；普通首次提交时若当前 revision ≠ task 创建时的 `base_revision`（且 `sourceGeneration` 仍匹配），不直接 apply，而是按 §9.3 规则 8 创建 successor task 并取消旧 task。compaction/replay 按 §2.1 的规则使用各阶段开始时捕获的最新 revision，其他 target 的合法 revision 增长本身不使原 proposal stale，但该阶段仍只能从实际最新 revision 创建下一 revision。
6. 每个 task phase 使用稳定的 event group identity；同一 group 内的 `patch_id` 以及非 null `result_revision` 都有唯一约束。capacity-blocked 审计与最终 replay 是两个稳定 phase/group，因此允许同一原 patchId 在两组中分别出现 deferred 与最终 decision。相同 phase/task/patchId 的重复 delivery、进程恢复或提交结果不确定时，先读取既有终态：已提交则返回原结果，未提交才继续；不得产生第二组 events、第二个 state revision、重复 snapshot 或重复 cursor 推进。compaction、proposal replay、system cleanup 与 cursor-only revision 遵守同一原则。resume 创建的新 maintenance child task 拥有独立 `task_id`（见 §9.3 规则 9），其 event-group identity 自然与旧 task 不同，不受旧 task 幂等终态约束。
...
5. `stage` 枚举按 `task_type` 区分：normal task 使用 `pending | proposing | proposal_persisted | capacity_blocked | replaying_original_proposal | succeeded | replay_failed`；maintenance task 使用 `pending | compacting | compaction_applied | compaction_failed`。`target_halted` 不是 task stage，只是 per-target status。maintenance task 的 `parent_task_id` 必须指向来源 normal task，并持久化 `resume_epoch`（首次为 0，每次 resume 递增）；normal task 在 `capacity_blocked` 阶段的 `stage_payload` 必须至少持久化 `persistedProposal`（完整原 proposal，供 replay 读取）、`maintenanceTaskId`（当前关联的 maintenance task ID）和 `resumeEpoch`（当前 resume 轮次）。`task_payload` 在 task 创建时固化 Proposer 输入后不可变，Proposer 运行时输出的 proposal 不写入 `task_payload`，只写入可变的 `stage_payload`。
...
7. `task_id` 与 `dedupe_key` 在任务首次创建后保持稳定。相同调度事实的重复 wake-up 必须命中同一 dedupe key 并读取既有 task，不得新建第二行；normal task 的 key 至少绑定 generation、target、`cursor_before` 与 `target_message_id`，maintenance task 还绑定 `parent_task_id`、阻塞 section 与 `resume_epoch`（首次为 0，每次 resume 递增），system cleanup task 绑定确定性 cleanup 边界。key 的具体编码由实现集中定义，不进入 Prompt。
8. **Successor task（revision stale 时创建）**：当 normal task 首次提交时发现当前 revision ≠ task 创建时的 `base_revision`（且 `sourceGeneration` 仍匹配），不得直接 apply，也不得简单失败。系统取消旧 task（status=`cancelled`），创建 successor task：新 `task_id`、新 `dedupe_key`、`predecessor_task_id` 指向旧 task。successor task 以当前最新 revision 作为新 `base_revision`，重新读取 state、重新组装 envelope 并重新调用 Proposer。旧 task 的 `stage_payload.persistedProposal`（如有）不复用。successor task 的 `dedupe_key` 必须与旧 task 不同（例如加入 predecessor_task_id 或新 base_revision）。readOnlyContext 可能已因其他 target 的 revision 而变化，因此必须重新组装 envelope。
9. **Maintenance task resume 创建新 child task**：当 maintenance task 已进入终态（`compaction_applied`/`compaction_failed`）且 parent normal task 仍处于 `capacity_blocked` 时，resume 不复用原 maintenance task，而是创建新 maintenance child task：新 `task_id`、`resume_epoch` = 前一个 + 1、`parent_task_id` 指向同一 normal task。新 child task 的 `dedupe_key` 包含新 `resume_epoch`，因此不会命中旧 task 的幂等终态。parent normal task 的 `stage_payload.maintenanceTaskId` 更新为新 child task ID。原 maintenance task 的终态保留用于审计，不被新 child task 覆盖。
...
调试信息只记录结构化元数据（taskId、targetKey、reason、attempt、reject_reason 等），用 `logger` 输出到应用日志，不进表。禁止将完整 raw prompt、完整 state diff 或完整 message content 写入 append-only 应用日志，因为这些日志无法按用户或消息精确删除，与 [write-protocol.md](write-protocol.md) §5.4 的 privacy hard delete 要求冲突。如需持久化完整调试 payload 用于离线分析，必须使用可按 `(user_id, preset_id)` 索引和删除的受控 debug 存储表，并在 privacy hard delete 时一并清除。
...
Forget/correction 的 source suppression tombstone 是独立于 `memory_state`、跨 `sourceGeneration` 保留的 durable sidecar（语义见 [write-protocol.md](write-protocol.md) §5.2）：
...
GapBridge omitted、projection lag 等 context 质量诊断是独立于 `memory_state`、semantic event 和 Memory ops log 的持久化 sidecar（语义见 [rendering-and-context.md](rendering-and-context.md) §2.1）：
