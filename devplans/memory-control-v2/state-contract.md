...
| `task.trigger`                        | `{ type: "lengthBudget", limit }`   | `limit` 是目标 section 的 item 数量上限                          |
...
- `patches` 数组中每个 patch 含 `op`、`path`（仅 scene 字段操作）/`itemId`/`itemIds`（按 §4 必填规则）、`value`（按 §4 必填规则，`todos` 的 `addItem`/`updateItem` 可选含 `expiresAt` 结构化表达式）、`evidenceKind`（§3 枚举）。除 `mergeItems` 外，每个 patch 还必须含 `evidenceRefs`（至少 1 项，每项含 `messageId` integer 和 `quote` string max 80 字符）。
...
| `milestones`                | 50            | 暂缓新增，触发 compaction task；失败后最终拒绝 |
| `worldFacts`                | 15            | 暂缓新增，触发 compaction task；失败后最终拒绝 |
| `userProfile`               | 40            | 暂缓新增，触发 compaction task；失败后最终拒绝 |
| `assistantProfile`          | 40            | 暂缓新增，触发 compaction task；失败后最终拒绝 |
| `relationship`              | 20            | 暂缓新增，触发 compaction task；失败后最终拒绝 |

`recentEpisodes` 的滑动窗口由 Reducer 在每次 apply 后执行：如果 item 数 > 5，移除最旧的（按 `createdAtMessageId` 排序），并记录 system cleanup event，禁止 silent delete。
...
Todo 到期不再删除或跨 section 迁移。Reducer 将同一 item 原位更新为 `status=overdue` 并设置 `becameOverdueAt`；该持久化变化记录 `system_cleanup: todo_became_overdue`。`todos` 的容量门只统计 `status=active` 的 items；overdue items 不触发 compaction，Renderer 对其使用独立的 `maxRenderedItems/maxRenderedChars`。

其它 section 溢出时不立即把当前消息视为已处理。Reducer 先记录 `deferred` 事件并触发 `compactionProposer` 维护任务。维护任务只能通过 `mergeItems + evidenceKind: "memory_compaction"` 合并同一正式 section 下的重复或高度重叠 item，不能使用通用删除，也不能把新事实写入长期记忆。compaction task 有界执行：同一 section 对同一阻塞窗口最多尝试 1 次。若维护任务 `accepted` 并释放容量，原 section 在下一次 tick 重新处理同一消息窗口（`deferred` 已阻止 cursor 推进，无论同 tick 是否存在 `accepted` patch，见 [write-protocol.md](write-protocol.md) §3）；若维护任务返回 `unable_to_compact`，触发 halt；若维护任务 `accepted` 但合并后仍超限，原新增 patch 最终 `rejected: length_budget_exceeded`，触发 halt；若维护任务技术性失败（`error`），按 [write-protocol.md](write-protocol.md) §3.1 error 恢复策略处理。resume 重置 compaction 的有界执行计数。
