  ...
  "readOnlyContext": {
    "current": {
      "scene": {
        "location": {
          "value": "屋顶",
          "evidenceRef": { "messageId": 118, "quote": "我们去屋顶吧" },
          "updatedAtMessageId": 118
        },
        "time": {
          "value": "深夜",
          "evidenceRef": { "messageId": 118, "quote": "都这么晚了" },
          "updatedAtMessageId": 118
        },
        "mood": {
          "value": "雨后安静",
          "evidenceRef": { "messageId": 119, "quote": "雨停以后好安静" },
      ...
      "todos": [
        {
          "id": "todo:1",
          "text": "归还橡皮",
          "evidenceRefs": [{ "messageId": 112, "quote": "明天提醒我把橡皮还给她" }],
      ...
      "standingAgreements": [
        {
          "id": "agreement:1",
          "text": "沉默时先开口说明状态",
          "evidenceRefs": [{ "messageId": 116, "quote": "以后沉默的时候先说一声" }],
      ...
      "relationship": [
        {
          ...
          "evidenceRefs": [{ "messageId": 60, "quote": "我一般慢热，熟了才会比较依赖人" }],
...
- `evidenceRefs.messageId` 对 Proposer 的语义：writableState 和 readOnlyContext 中旧 item 的 `evidenceRefs.messageId`（如上例的 110、80、112、116、60、45、30）通常不在当前 tick 的 `evidenceMessages` 中——它们来自更早的 tick。Proposer 只应读 `quote` 做上下文理解（溯源语气、辅助去重），不应尝试通过 messageId 回查旧消息。messageId 对 Reducer 有意义（校验新 patch 的 quote），对 Proposer 是只读元数据。
...
| `readOnlyContext`                     | `current.scene`、`working.todos` active 子集、`working.standingAgreements`、`working.recentEpisodes`、`longTerm.milestones`、非目标 core sibling arrays | 只用于防止误合并，不能作为 evidenceRefs 来源；范围由目标 section/path 固定决定               |
  ...
  "readOnlyContext": {
    "current": {
      "scene": {
        ...
        "mood": {
          "value": "安静",
          "evidenceRef": { "messageId": 119, "quote": "这里好安静" },
          "updatedAtMessageId": 119
        },
        "note": { "value": null, "evidenceRef": null, "updatedAtMessageId": null }
      }
...
| `compactionProposer`   | 被预算阻塞的 section/path      | `current.scene`、`working.todos` active 子集、`working.standingAgreements`、`working.recentEpisodes`、`longTerm.milestones`、非目标 core sibling arrays  |
...
- `compactionProposer` 的 schema 额外限制：只能输出 `mergeItems`，且 `evidenceKind` 只能是 `memory_compaction`。输出 `addItem`、通用删除、跨 section 合并、跨 core path 合并或 `user_correction` 均非法。
