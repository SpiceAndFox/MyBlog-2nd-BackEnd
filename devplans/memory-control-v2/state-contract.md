...
`expiresAtMessageId` 过期清理（见 [write-protocol.md](write-protocol.md) §1.3 步骤 7）同样不产生事件行。这两类"自然遗忘"不产生审计事件是刻意取舍：它们是确定性策略驱动的例行清理，记录会淹没 events 表的 patch 决策行。如需排查某 item 为何消失，可从 `memory_state` 快照和 `createdAtMessageId`/`expiresAtMessageId` 推断。
