...
- maintenance task 的 `parent_task_id` 必须指向来源 normal task；normal task 在 `capacity_blocked` 阶段的 `stage_payload` 必须持久化当前 `maintenance_task_id`。
...
- stale revision/cursor 执行结果写 `stale_result` ops log 并丢弃，不得覆盖新 state；replay 的 stale 判定见 [write-protocol.md](write-protocol.md) §2.1 步骤 7（只看 target cursor、proposal 活动性、引用 item 存在性和 schema/source 兼容性，不看全局 revision）。
