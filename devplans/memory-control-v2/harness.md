...
- `memory_compaction` 的 `value.text` 不得引入 source items 未表达的新事实——此约束由 compactionProposer prompt 承担（[proposer-prompt.md](proposer-prompt.md) §2.3/§4.6），Reducer 不做语义检测，仅 LLM smoke 覆盖。
