...
- `active todos` 是由 todo 状态字段定义的完整子集；fixture 应覆盖 active item 出现、非 active item 不出现在该子集的行为。
...
- `error`/`unable_to_decide` 落 `chat_memory_ops_log`（[state-contract.md](state-contract.md) §9.2），不落 events；经恢复策略推进 cursor 时，未覆盖 message 写入 `chat_memory_skipped_messages`。
- `unable_to_decide` 首次后 `meta.recovery.awaitingContextExpansion` 置 true，下一 tick 发扩大 contextWindow；二次仍 `unable_to_decide` 推进并写 skipped。
- `error` 连续 3 次后推进 cursor 并写 skipped（reason=`max_retry_exceeded`），`consecutiveErrors` 清零。
...
- `memory_compaction` 的 evidenceRefs 必须来自维护模式 `writableState` source items 的既有证据。
...
### 3.7 Provider Adapter 与回填

- mock adapter 返回 `status: "ok"` → output 交 Reducer，patch 决策落 events。
- mock adapter 返回 `status: "error", reason: "safety_policy_blocked"` → 不交 Reducer，落 ops_log（outcome=`safety_policy_blocked`），cursor 不推进；连续达阈值后推进并写 skipped（reason=`safety_policy_blocked`）。
- mock adapter 返回 `status: "error", reason: "output_schema_invalid"` → 不重试同输入，直接推进 cursor 并写 skipped。
- mock adapter 返回 `status: "error", reason: "llm_call_failed"` → 递增 `consecutiveErrors`，下一 tick 自然重试；达 3 次后推进并写 skipped（reason=`max_retry_exceeded`）。
- `unable_to_decide` 首次 → `awaitingContextExpansion=true`、cursor 不推进；下一 tick contextWindow 扩大；二次仍 `unable_to_decide` → 推进 cursor，未覆盖 message 写 skipped 表（reason=`unable_to_decide`）。
- skipped 表 `resolved` 默认 FALSE；回填工具重跑 message 并产生 accepted patch 后置 TRUE，不删行。
- `accepted`/`rejected`/`noop`/`deferred` 均重置 `consecutiveErrors` 与 `awaitingContextExpansion`。
- Reducer 永远只收到 `status: "ok"` 的 output，不处理空输出或伪造输出。
...
- Provider Adapter 返回 `safety_policy_blocked` -> 落 ops_log，cursor 不推进；连续达阈值后推进并写 skipped，主聊天仍从旧 `memory_state` 实时 render。
- skipped 表 backlog 中未覆盖 message 经回填工具重跑并产生 accepted patch后 `resolved=TRUE`。
