...
- `deferred` 优先于 `rejected` 阻止 cursor 推进：同 section 有 `deferred` 且无 `accepted` 则不推进，即使存在 `rejected` 行。
...
- `error` 连续 3 次后触发全局 halt（`meta.halted=true`），聊天接口拒绝新消息；`consecutiveErrors` 保留不清零，等手动 resume 时重置。
...
- mock adapter 返回 `status: "error", reason: "safety_policy_blocked"` → 不交 Reducer，落 ops_log（outcome=`safety_policy_blocked`），cursor 不推进；连续达 3 次后触发全局 halt。
- mock adapter 返回 `status: "error", reason: "output_schema_invalid"` → 不重试同输入，直接触发全局 halt（持续性错误，重试同输入大概率仍非法）。
- mock adapter 返回 `status: "error", reason: "llm_call_failed"` → 递增 `consecutiveErrors`，下一 tick 自然重试；达 3 次后触发全局 halt。
...
- 全局 halt 后 `meta.halted=true`，聊天接口拒绝新消息；resume CLI/API 重置 `halted=false` 和所有 `consecutiveErrors` 后入队正常 tick。
...
- 连续错误触发全局 halt 后，手动 resume 从 cursor 位置继续处理，不跳过任何消息。
...
- assistant 修正已有 core item -> `correctItem + assistant_correction` 接受（所有 core 子数组均可）。
- core/userProfile 达到上限 -> 新增被 deferred -> compaction 合并重复项 -> 下一 tick 可写入新 item。
- compaction 无安全合并项 -> 最终 length_budget_exceeded，不推进坏写入。
- Provider Adapter 返回 `safety_policy_blocked` -> 落 ops_log，cursor 不推进；连续达阈值后触发全局 halt，聊天接口拒绝新消息；resume 后从 cursor 位置继续。
