...
- normal/maintenance task 的 proposal-time envelope 与 evidence metadata 必须写入 immutable `task_payload`；retry/restart 不得从变化后的 recent window 临时重组同一个 task input。
...
- 可推进终局中的 `accepted`/`noop`/普通 `rejected` 推进对应 target cursor，并在一个事务形成 `baseRevision + 1`、event group/events、完整 post-state snapshot、task 终态和 healthy target status。
...
- 混合 `accepted`+`deferred` 的原子提交与 revision 分配由第 5 批统一替换；本批不锁定专用状态机，只断言任何实际提交的 state revision 都必须拥有同号完整 snapshot，未提交 state/cursor 时不得伪造 revision。
...
- compaction 成功执行 mergeItems 但合并后仍超限 → Reducer 记录 `rejected: length_budget_exceeded` → halt 对应 target，cursor 不推进。
- `length_budget_exceeded` 是唯一会 halt 对应 target 的 reject reason，其他 reject reason 仍推进 cursor。
...
- `maxItems` 未超但 apply 后 `maxRenderedChars` 超限，以及 `maxRenderedChars` 未超但 `maxItems` 超限，均进入 `deferred: length_budget_exceeded`；maintenance trigger 分别记录正确的 `dimension` 和 `limit`。
- `updateItem` 等非 add patch 扩大语义文本并导致 `maxRenderedChars` 超限时同样经过容量门，不得只在 `addItem` 上检查。
- `maxRenderedChars` 按 Unicode code points 只统计 Renderer 可能输出的语义文本；item 的 quote/evidenceGroups/hash/ID/provenance 与 Renderer 标题、连接词、模板标点不计。
...
- `recentEpisodes` 的滑动窗口滚出是确定性例外，完整规则在第 6 批与领域生命周期一起落地；本批 `recentEpisodes` 与其他 item section 一样走基础 `maxItems + maxRenderedChars` 容量门。
- item section 超出长度预算时先 `deferred` 并创建 compaction task。compaction 返回 `unable_to_compact` 或合并后仍超限时只 halt 对应 target。
...
- scene/todo/recentEpisodes 的具体 cleanup type 和 lifecycle apply 断言留到第 6 批；本批只断言任何确定性持久化变化都产生通用 system cleanup event、revision 和完整 snapshot，不得静默修改。
- overdue todo 仍可由 `completeTodo`/`cancelTodo` 终止；active 容量只统计 active items，overdue items 不触发 compaction、不得 merge。
- `current.previousScene` 保持单值衍生字段且不拥有 section/cursor；scene 到期/替换时的具体 cleanup type 与 lifecycle 断言留到第 6 批。
...
- resume 指定 target：重置其 status/error count/nextRetryAt 并重新排队可恢复 task；不修改 state/revision/snapshot，也不重置其他 targets。
...
- 初始化 state 时同时写 revision 0 完整 snapshot；每个成功 revision N 恰好一份同号完整 post-state snapshot，不额外写 pre-state snapshot。
...
- system cleanup 修改持久化 state 时必须写 `group_kind=system_cleanup`、`decision=system_cleanup` 和完整 normalized operation；本批不绑定具体 scene/todo cleanup 类型。
- 从最新合法 snapshot replay 后续 event groups 可恢复相同 state/cursors；replay 不调用 LLM，并拒绝 revision 断层、cursor 不连续、schema 不兼容或 task/target 不一致的 event group。
- snapshot/state 损坏时优先恢复到最新合法 snapshot；必要时从 raw messages rebuild，完整 rebuild 流程留到第 9 批。
- 进程重启会从数据库读取 queued/running/retry_wait task 并从持久化 stage 继续；进程内队列、计数器或 flag 丢失不影响恢复。
- stale revision/cursor 执行结果写 `stale_result` ops log 并丢弃，不得覆盖新 state；后续 proposal replay 细节留到第 5 批。
...
- compaction 无安全合并项 -> 返回 unable_to_compact -> halt 对应 target；resume 指定 target 后重新尝试。
