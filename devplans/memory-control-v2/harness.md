...
```json
{
  ...
  "initialState": {
    ...
    "meta": { "halted": false, "perSectionCursor": { "todos": 120 }, "recovery": {} }
  },
  "inputEnvelope": {
    "task": {
      "tickId": 12345,
    ...
    "observedMessages": [{ "id": 121, "role": "user", "contentKind": "raw", "content": "明天提醒我把橡皮还给她" }]
  },
  "proposerOutputs": [
    {
      "tickId": 12345,
...
- 普通写入 patch accepted 后，Reducer 将该 patch 的 `evidenceRefs` 包装为一个 `evidenceGroup`。
...
- 同一 Proposer、同一 target、同一 `memory_state` 下，即使 observed messages 内容不同，`readOnlyContext` 的 section/path 形状也必须一致；只允许 `task.observedMessageIds` 和 `observedMessages` 随消息窗口变化。
...
- `core.addItem + scene_change` 拒绝。
- `core.addItem + long_term_fact` 接受，包含单窗口内证据明确的行为推断。
- `core.updateItem + long_term_fact` 拒绝。
- `core.updateItem + user_correction/assistant_correction` 接受。
...
- `memory_compaction` 的 `value.text` 不得引入 source items 未表达的新事实——此约束由 compactionProposer prompt 承担（[proposer-prompt.md](proposer-prompt.md) §2.4/§4.6），Reducer 不做语义检测，仅 LLM smoke 覆盖。
- Proposer 输出非 target section 时记 `output_schema_invalid` 并触发 halt；patch 指向非 target path 时拒绝，且非 target cursor 不推进。
...
- `expiresAt` 缺省时 item 无 `expiresAtTime`（null），不触发过期清理。
...
- mock adapter 返回 `status: "error", reason: "safety_policy_blocked"` → 不交 Reducer，落 ops_log（outcome=`safety_policy_blocked`），cursor 不推进，`consecutiveErrors` 递增；达 3 次时第 3 行 outcome 记为 `halted` 并设置 `meta.halted=true`。
- mock adapter 返回 `status: "error", reason: "output_schema_invalid"` → 不重试同输入，直接触发 halt（持续性错误，重试同输入大概率仍非法）。
- mock adapter 返回 `status: "error", reason: "llm_call_failed"` → 不交 Reducer，落 ops_log（outcome=`llm_call_failed`），cursor 不推进，`consecutiveErrors` 递增，下一 tick 自然重试；达 3 次时第 3 行 outcome 记为 `halted` 并设置 `meta.halted=true`。
