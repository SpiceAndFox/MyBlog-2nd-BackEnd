...
2. **Read-only memory context**：帮助 Proposer 理解当前对话的背景状态，例如当前场景、人物状态、已有待办、近期经历、里程碑和固定 core 切片。它只能读，不能写，不能作为新 patch 的直接证据。
...
```js
{
  ...
  readOnlyContext: {
    ...
    longTerm: { milestones: [], coreContext: {} }
  },
  evidenceMessages: [
...
- `readOnlyContext` 的 section/path 范围由 Proposer 类型和目标 section/path 固定决定，不能根据 observed messages 的语义临时增减。
- 固定范围以完整 segment 为单位：一个 section/path 一旦被纳入本 Proposer 输入，就全量输入该 segment 的当前 items，不做 last N、相似度筛选或按 blocked patch 语义筛选。
- `active todos` 这类名字表示一个由状态字段定义的完整子集；不要再对 active 子集做数量截断。
- target section/path 的当前状态只放在 `writableState`；如果某个固定背景范围概念上包含 target，本次 envelope 仍以 `writableState` 为准，不在 `readOnlyContext` 重复放一份。
...
固定 read-only / maintenance 输入范围：

| Proposer               | Writable target                | 固定输入范围                                                                                                                                                                                                                             |
| ---------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `currentStateProposer` | `scene`, `participants`        | `working.recentEpisodes`、`working.todos.active`、`longTerm.coreContext.relationship`；用于理解延续场景和避免无变化时重复改写。旧 `scene` / `participants` 来自 `writableState`。 |
| `todoProposer`         | `todos`                        | `current.scene`、`current.participants`、`working.recentEpisodes`、`longTerm.coreContext.relationship`、`longTerm.coreContext.userProfile`；用于识别指代、承诺和长期偏好。 |
| `episodeProposer`      | `recentEpisodes`, `milestones` | `current.scene`、`current.participants`、`working.todos.active`、`longTerm.coreContext.relationship`、`longTerm.coreContext.userProfile`、`longTerm.coreContext.assistantProfile`；这是 read-only context 最宽的普通 Proposer。已有 episode/milestone 由 `writableState` 提供，用于去重和晋升判断。 |
| `coreProposer`         | `core`                         | `current.scene`、`current.participants`、`working.recentEpisodes`、`longTerm.milestones`、非目标 core sibling arrays；用于判断长期事实、用户修正、去重，以及排除临时状态。目标 core 子数组来自 `writableState`。 |
| `compactionProposer`   | 被预算阻塞的 section/path      | `maintenance.sourceItems` 使用目标 section/path 的既有 items 集合，不按数量或 blocked patch 文本语义筛选。`maintenance.sourceEvidenceMessages`、`maintenance.blockedPatchSummary`，再加固定背景：`current.scene`、`current.participants`、`working.todos.active`、`working.recentEpisodes`、`longTerm.milestones`、非目标 core arrays。背景范围由目标 section/path 决定，不由 blocked patch 文本决定。 |

`readOnlyContext` 可以相对充分，但必须由纯代码按上述固定范围从 `memory_state` 结构化组装。禁止把 Renderer 文本、旧 summary 或未分区的整块 `memory_state` 无差别塞入 Proposer。
...
3. compaction task 由同一 `userId/presetId` 串行队列执行，输入使用同一 envelope：`writableState` 只包含目标 section/path，`maintenance.sourceItems` 包含目标 section/path 的既有 items 全集，不做数量截断或 blocked patch 文本语义筛选。`maintenance.sourceEvidenceMessages` 包含 source item 既有证据对应的 raw messages，`readOnlyContext` 按 `compactionProposer` 的固定范围补充背景防止误合并。
...
`compactionProposer` 的输入不是普通最近对话窗口，而是 `mode: "maintenance"` 的 envelope。它仍然可以读取固定背景，但证据只能来自 source items 的既有证据。示例：
