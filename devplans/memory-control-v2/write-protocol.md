...
3. 为每个 eligible task 收集 observedMessageIds（lag 范围内的消息），并组装 Proposer input envelope（结构见 [state-contract.md](state-contract.md) §5）。
...
| `unable_to_decide` | 不推进（连续 2 次后推进） | Proposer 自认判断不了；连续 2 次后推进并记录 skipped event，避免永久卡死                    |
| `error`          | 重试 1 次后推进           | schema/llm/provider 技术性失败；仍失败则记录 error event 并推进，保留上一次稳定 state        |

`rejected` 推进的理由：Proposer 已处理消息，重跑同样输入大概率得到类似结果——`policy_not_allowed` 是系统性错误，`duplicate_item` 是确定性拒绝，`quote_not_found` 虽可能因 LLM 随机性下次摘录不同 quote，但不值得为此卡住整个 section。`length_budget_exceeded` 只有在 compaction 已尝试且失败后才作为最终 rejected 推进。

`deferred` 不推进：该 section 等待 compaction task 有界执行；compaction 成功后重新跑同一窗口，compaction 失败后改为最终 rejected 并推进。
...
- **Compaction task 失败**：记录 `error` 或 `rejected`，保留旧 state，不回退到全文摘要重写；原阻塞 section 按 §3 规则完成最终预算拒绝。
