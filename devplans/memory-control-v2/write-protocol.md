...
Proposer 按记忆族拆分调用，每个调用都必须使用 provider 支持的 schema-constrained structured output 强制输出 schema（见 [state-contract.md](state-contract.md) §5.5）。
...
9. **事件记录**：每个 patch 的决策写入 `chat_memory_events`（[state-contract.md](state-contract.md) §9）。
...
3. compaction task 由同一 `userId/presetId` 串行队列执行，使用 [state-contract.md](state-contract.md) §5.2 的 maintenance 模式 envelope。
...
| `noop`             | 推进        | Proposer 明确说无变化（不产生事件行，或产生 `decision=accepted, op=null` 占位行供审计对齐）  |
| `rejected`         | 推进        | patches 状态但全被拒（policy/quote/duplicate 等），或 compaction 无合并空间后的最终预算拒绝 |
| `deferred`         | 不推进      | 新增被长度预算阻塞，已触发 compaction task                                                  |
| `unable_to_decide` | 不推进      | Proposer 自认判断不了；按 §3.1 扩大上下文重试，仍无法判断后推进                              |
| `error`            | 不推进      | 技术性失败；按 §3.1 重试与升级，达到阈值后推进                                               |

`rejected` 推进的理由：Proposer 已处理消息，重跑同输入大概率得到类似结果——`policy_not_allowed` 是系统性错误，`duplicate_item` 是确定性拒绝，`quote_not_found` 虽可能因 LLM 随机性下次摘录不同 quote，但不值得为此卡住整个 section。`length_budget_exceeded` 只在 compaction 无合并空间后才作为最终 rejected 推进。
...
**error（技术性失败：LLM 调用失败 / schema 非法 / provider 拦截）**

- 不推进 cursor。下一 tick 该 section lag 仍超标，自然重试（tick 调度提供间隔，不是热循环）。
- 区分错误性质：`llm_call_failed`/`timeout`/`safety_policy_blocked` 视为瞬时，允许跨 tick 重试；`schema_invalid` 视为持续性（同输入大概率仍非法），不重试同输入，直接推进 cursor 并告警——schema-constrained output 正常情况下不应出现 `schema_invalid`，出现即 provider/schema 配置 bug。
- 升级阈值：同一 section 连续 3 次 error 后，推进 cursor 解锁、记录告警日志、保留上一次稳定 state。不需要独立"冻结"状态——cursor 推进后下次 tick 自然重新评估。

**unable_to_decide（Proposer 自认信息不足）**

- 不推进 cursor。重试时**向后扩大 contextWindow**（纳入更多已 cover 的历史消息作上下文），而非重发同输入——同输入必然同结果。
- 扩大 1 次（如 contextWindow +10）仍 `unable_to_decide` → 推进 cursor 并记录 skipped event，避免无限扩大。

**rejected（Proposer 已判断但被拒）**

- 推进 cursor（见 §3 表格）。不重试——重跑同输入大概率同结果。
- 不自动告警；rejected 模式靠人工查 `chat_memory_events` 发现。

**deferred（预算阻塞）**

- 不推进 cursor。触发 compaction task。compaction 成功释放容量 → 下一 tick 重跑同一消息窗口；compaction 返回 `noop`/`unable_to_decide`（无合并空间）→ 改 `rejected` 推进；compaction 技术性失败 → 按 error 策略处理。
