...
Provider output 先通过 Semantic Schema 校验。若任一 target section 为 `unable_to_decide`，整个结果用短事务写入 `stage_payload.unableResult` 并进入 `unable_result_persisted`，不得部分 compile 其他 section；首次 unable 生成 durable `expandedArtifact(publicInput + messageMeta)` 后重提整个联合 task，二次 unable 原子提交 cursor-only revision。只有不含 unable 的 compiler-ready Semantic IR 才写入 `stage_payload.semanticResult + semanticInputVariant`、进入 `semantic_result_persisted` 并运行 Compiler。

Compaction 的 `unable_to_compact` 同样写入 `unableResult` 且不进入 Compiler；lengthBudget 与 hygiene 后续终局分别按 §7 处理。

Compiler：

1. 按 `semanticInputVariant` 读取 immutable base artifact 或 durable expanded artifact，并始终复用 base ref map；
...
- item update 合并旧 provenance和本 change sources；scene set/correct 使用本次 compiled sources替换 field provenance；
...
| length-budget `unable_to_compact` | 不推进并 halt target |
| hygiene `unable_to_compact` | 不涉及 normal cursor，maintenance 以 `status=succeeded,stage=hygiene_noop|hygiene_skipped` 终结且 target 保持 healthy |
