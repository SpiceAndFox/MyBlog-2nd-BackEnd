...
- 持续性错误（`output_schema_invalid`）：不递增 `consecutiveErrors`、不重试同输入，直接推进 cursor 并写 skipped（reason=`max_retry_exceeded`）——schema-constrained output 正常情况下不应出现 `output_schema_invalid`，出现即 provider/schema 配置 bug，重试同输入大概率仍非法。
- 升级阈值（仅瞬时错误）：`consecutiveErrors` 达 3 后，推进 cursor 解锁、写 skipped 表、清零计数、保留上一次稳定 state。skipped reason 取最后一次 error 的 outcome：`safety_policy_blocked` 保留本名，`llm_call_failed` 归为 `max_retry_exceeded`。不需要独立"冻结"状态——cursor 推进后下次 tick 自然重新评估。
