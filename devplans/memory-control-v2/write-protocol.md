...
- `lagThreshold`（N）：检测频率。`lag = recent window 末端 message id - 该 section 的 coveredUntilMessageId`，lag >= N 时该 section eligible。
- `contextWindow`（M ≥ N）：发给 Proposer 的证据窗口大小。`observedMessageIds = [target - M + 1 ... target]`（向前截断到 1）。其中最旧的 M - N 条是已 cover 的历史消息，作为重叠上下文重发——evidenceRefs 可以引用 observedMessageIds 中的任意消息（含重叠部分），但 patch 应反映新信息。
...
| `unable_to_decide` | 不推进      | ops_log   | Proposer 自认判断不了；按 §3.1 扩大上下文重试，仍无法判断后推进并写 skipped                 |
| `error`            | 不推进      | ops_log   | Provider Adapter 返回 `status: "error"`（[state-contract.md](state-contract.md) §10）；按 §3.1 重试与升级，达到阈值后推进并写 skipped |
...
多 patch 部分接受：一个 section 一个 tick 的 `patches` 数组里可能多个 patch 独立校验，部分 `accepted` 部分 `rejected`。cursor 按 section 级聚合——有任一 `accepted` 即按 `accepted` 推进，被拒 patch 的 `reject_reason` 仍各自落 event 行供审计。
...
- tick orchestrator 不把 error 结果交给 Reducer，直接写 ops_log（[state-contract.md](state-contract.md) §9.2）并递增 `meta.recovery[section].consecutiveErrors`。cursor 不推进，下一 tick 该 section lag 仍超标，自然重试（tick 调度提供间隔，不是热循环）。
- 区分错误性质（由 adapter 识别）：`llm_call_failed`/`safety_policy_blocked` 视为瞬时，允许跨 tick 重试；`output_schema_invalid` 视为持续性（同输入大概率仍非法），不重试同输入，直接推进 cursor 并写 skipped（reason 归为 `max_retry_exceeded`）——schema-constrained output 正常情况下不应出现 `output_schema_invalid`，出现即 provider/schema 配置 bug。
- 升级阈值：`consecutiveErrors` 达 3 后，推进 cursor 解锁、写 skipped 表、清零计数、保留上一次稳定 state。skipped reason 取最后一次 error 的 outcome：`safety_policy_blocked` 保留本名，`llm_call_failed` 归为 `max_retry_exceeded`。不需要独立"冻结"状态——cursor 推进后下次 tick 自然重新评估。
...
- 扩大 1 次仍 `unable_to_decide` → 推进 cursor、写 skipped 表（reason=`unable_to_decide`）、清 `awaitingContextExpansion`，避免无限扩大。
...
`core` 的新增只接受 `long_term_fact` 一种 evidenceKind：用户或设定文本明确表达的长期事实，包括用户描述（"我叫小明"、"我讨厌早起"）与 assistant 设定人格（"这个世界有魔法"、角色性格底色）。对已有 core item 的改写只能基于 `user_correction`，走 `correctItem`/`updateItem`（见 [state-contract.md](state-contract.md) §6 policy table）——`addItem + user_correction` 不再允许，因为"修正"语义上隐含"纠正已存在的东西"，应走更新 op；若用户纠正后要新增一条全新 core item，本质就是一条新的 `long_term_fact`。
...
> 跨 tick 重复模式累积晋升（N=3/K=2）需要确定性 ledger + 结构化标签匹配（非语义匹配）才能可靠实现，首版不做，留作未来探索。
...
- **Section 连续多次 error**：按 §3.1 升级阈值推进并告警，保留旧 state，其它 section 继续推进。
