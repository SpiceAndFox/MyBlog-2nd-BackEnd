...
2. 为每个 eligible target 创建一个 `eligibleTask`；每个 normal target 唯一映射到一个专用 Proposer。
...
6. **长度预算**：按 [state-contract.md](state-contract.md) §8 校验各 item section 的 `maxItems + maxRenderedChars`，只统计 Renderer 可能输出的语义文本。任一基础维度超限时返回 `deferred: length_budget_exceeded` 并触发 compaction task；`recentEpisodes`、previousScene、overdue todo 的确定性例外留在领域生命周期批次定义。
7. **领域生命周期**：Reducer 自行扫描到期 todo，将同一 item 原位更新为 `status=overdue` 并设置 `becameOverdueAt`，记录 `system_cleanup: todo_became_overdue`；scene 到期时把完整旧值写入 `current.previousScene` 并清理 current scene，记录对应 cleanup event。两者都不创建新 section 或 cursor。
...
9. **事件记录**：每个 patch 的决策写一行 `chat_memory_events`（[state-contract.md](state-contract.md) §9.1）；`noop` 写一行占位（`decision=noop`，`patch_id` 为 null）。一个 section 一个 tick 可能有多个 patch，因此可能有多行 event。target 级 cursor 推进是派生的：按 `target_key` 聚合 event，并结合整个 proposal 的 section status 与 ops outcome 判断（见 §3）。
...
1. normal proposal 中的 item patch 经模拟 apply 后，目标 section 将超过 `maxItems` 或 `maxRenderedChars`；不只检查 `addItem`，会增长渲染文本的 `updateItem` 等操作同样受容量门约束。
2. Reducer 不立即最终拒绝，也不推进该 target cursor，而是记录 `decision: "deferred"`（[state-contract.md](state-contract.md) §9.1，`deferred` 不填 `reject_reason`），关联 `maintenance_task_id`，并创建一个 compaction task。
3. compaction task 与普通 task 共用同一 `userId/presetId` 串行队列（见 §2），使用 [state-contract.md](state-contract.md) §5.2 的 maintenance 模式 envelope；trigger 的 `dimension` 记录本次阻塞来自 `maxItems` 还是 `maxRenderedChars`。其 `targetKey` 只关联来源 normal target；`targetMessageId` 只复制来源 normal proposal 的阻塞边界，用于关联、幂等和后续 replay。两者均不表示 compaction 拥有或推进 raw-message cursor，compaction 也不据此读取 raw messages。
...
6. 如果 compaction 成功释放容量，原 section 保持 cursor 不变，在下一次 tick 重新处理同一消息窗口（`deferred` 已阻止 cursor 推进，无论同 tick 是否存在 `accepted` patch，见 §3）。如果 compactionProposer 返回 `unable_to_compact`（无安全合并空间），触发 halt。如果 compaction `accepted` 但合并后仍超限，原 capacity-blocked patch 最终 `rejected: length_budget_exceeded`，触发 halt。如果 compaction 发生技术性失败（LLM 调用/schema/provider），按 §3.1 error 恢复策略处理。

compaction task 有界执行：同一 section 对同一阻塞窗口最多尝试 1 次。resume 重置该有界执行计数，允许重新尝试同一阻塞窗口。
...
`accepted`/`rejected`/`deferred`/`noop` 是 Reducer 的 patch 决策，落 `chat_memory_events`（[state-contract.md](state-contract.md) §9.1）；`error`/`unable_to_decide`/`unable_to_compact` 发生在 patch 产生之前（Provider Adapter 层或 Proposer 自身），落 `chat_memory_ops_log`（[state-contract.md](state-contract.md) §9.2）。`error` 达阈值后触发该 `userId/presetId` 的 halt；`unable_to_decide` 扩窗口重试一次后仍无法判断则推进 cursor；`unable_to_compact` 直接触发 halt。
...
| `rejected`（`length_budget_exceeded`） | 不推进      | events  | compaction 无合并空间或合并后仍超限；触发 halt（见 §3.1）                                                                                     |
...
| `unable_to_compact`                    | 不推进      | ops_log | compactionProposer 判定无安全合并空间；触发 halt（见 §3.1）                                                                                   |
...
一个 section 的 `patches` 数组里可能多个 patch 独立校验，部分 `accepted`、部分普通 `rejected` 时可以推进；有任一 `deferred` 时不推进。`deferred` 阻止推进是因为该 target 需要在 compaction 后重跑整个消息窗口——已 `accepted` 的 patch 已写入 state，重跑时 Proposer 会看到更新后的 `writableState` 并对已处理内容输出 `noop`，不会重复写入。被拒 patch 的 `reject_reason` 仍各自落 event 行供审计。

**混合 accepted+deferred 时的 reprocess 语义**：当同一 target 同一 tick 既有 `accepted` 又有 `deferred` patch 时，cursor 不推进（`deferred` 阻止推进）。compaction 成功后，target 重新处理同一消息窗口——Proposer 看到更新后的 `writableState`（已 `accepted` 的 patch 已在 state 中），对已处理内容输出 `noop`，对原 `deferred` 的 patch 重新输出（此时 compaction 已释放容量，可以 `accepted`）。compaction 返回 `unable_to_compact` 或合并后仍超限时，触发 halt（见 §3.1）。

`deferred` 不推进：该 target 等待 compaction task 有界执行；compaction 成功后重新跑同一窗口，compaction 无合并空间或合并后仍超限时触发 halt。
...
- tick orchestrator 不把 error 结果交给 Reducer，直接写 ops_log（[state-contract.md](state-contract.md) §9.2）并按错误性质分流。cursor 不推进，下一 tick 该 target lag 仍超标，自然重试（tick 调度提供间隔，不是热循环）。
- 瞬时错误（`llm_call_failed`/`safety_policy_blocked`）：递增 `meta.recovery[targetKey].consecutiveErrors`，允许跨 tick 重试。
...
resume 做的事：重置 `meta.halted=false` 和所有 `meta.recovery[targetKey].consecutiveErrors=0`，入队一个正常 tick。下一 tick 各 target 的 lag 会很大（因为 cursor 停在旧位置），Observer 按"cursor 之后第 N 条消息"取窗口，正常走 pipeline。如果错误已修复 → 正常推进；如果没修复 → 再次 halt。

**unable_to_decide（Proposer 自认信息不足）**

- tick orchestrator 写 ops_log（outcome=`unable_to_decide`），置 `meta.recovery[targetKey].awaitingContextExpansion = true`，不推进 cursor。
- 下一 tick Observer 读到 `awaitingContextExpansion` → 发扩大的 contextWindow（如 +10）。
- 扩大 1 次仍 `unable_to_decide` → 推进 cursor、清 `awaitingContextExpansion`。
...
- 不推进 cursor，记 events（decision=`deferred`）。触发 compaction task。compaction 成功释放容量 → 下一 tick 重跑同一消息窗口；compaction 返回 `unable_to_compact` → 触发 halt；compaction `accepted` 但合并后仍超限 → 最终 `rejected: length_budget_exceeded`，触发 halt；compaction 技术性失败 → 按 error 策略处理。

**unable_to_compact（compactionProposer 判定无安全合并空间）**

- tick orchestrator 写 ops_log（outcome=`unable_to_compact`），触发 halt。不重试。
- resume 后重置 halted + compaction 有界执行计数，重新尝试。

**length_budget_exceeded（compaction accepted 但合并后仍超限）**

- Reducer 记录 `rejected: length_budget_exceeded`，tick orchestrator 触发 halt。
- resume 后重置 halted + compaction 有界执行计数，重新尝试。
...
长期 section 的去重由对应 Proposer 在各自 `writableState` 范围内完成；Reducer 不做文本相似度去重。若重复 item 溜进来，由 `compactionProposer` 在同一正式 section 内用 `mergeItems` 兜底，禁止跨 section 合并。
