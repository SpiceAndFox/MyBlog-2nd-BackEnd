...
8. **apply**：通过校验的 patch 应用到 state，生成新 state。普通非 `mergeItems` item patch 的 `evidenceRefs` 包装为一个 `evidenceGroup`；`mergeItems` 继承 source items 的 `evidenceGroups`。含 `expiresAt` 的 todo patch 将 step 1 计算出的 `expiresAtTime` 写入 item。
