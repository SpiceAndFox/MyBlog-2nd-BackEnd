...
3. 恢复时明确产生一次“Memory 已追平到相应 boundary”的恢复通知；恢复通知的 delivery 状态持久化到 [state-contract.md](state-contract.md) §9.10 的 `chat_memory_recovery_notifications` 表，保证恰好一次通知。清除 active 告警与写入恢复状态使用同一已持久化结果，不能先宣称恢复再异步清状态。
