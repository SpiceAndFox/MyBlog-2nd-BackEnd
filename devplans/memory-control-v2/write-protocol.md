...
Proposer 自评 `evidenceKind`（见 [state-contract.md](state-contract.md) 的 Evidence Kind 合法值），这是 Reducer 做 policy gate 的依据。Proposer 不输出自由置信度分数。

如果某个 Proposer 没输出目标 section 的结果，Reducer 视为该 section `error`，不猜测推进。

具体 function calling schema 见第 9 节，prompt 设计见 [proposer-prompt.md](proposer-prompt.md)。
