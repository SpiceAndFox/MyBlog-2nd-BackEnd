...
3. **Evidence messages**：用于写入校验的 raw messages。普通模式下是最近对话 raw messages；维护模式下是目标 source items 既有 `evidenceRefs` 对应的 raw messages。

统一 envelope 形态。`task.mode` 是判别字段，决定 `trigger`、`writableState` 和 `evidenceMessages` 的具体语义：
...
```js
{
  task: {
    ...
    trigger: { type: "lagThreshold" | "lengthBudget" }
  },
  writableState: {
    // 只放本次 target section/path 的当前状态；item 一律保留 evidenceRefs 和 evidenceKind
  },
  readOnlyContext: {
...
- 普通模式的 `task.trigger` 为 `{ type: "lagThreshold" }`。普通写入 patch 的 `evidenceRefs.messageId` 必须来自 `evidenceMessages`，不得引用 `readOnlyContext` 中 item 的历史证据来证明新事实。
- 维护模式的 `task.trigger` 为 `{ type: "lengthBudget", limit, blockedPatchSummary }`。维护 patch 的 `evidenceRefs` 只能复制 `writableState` 中 source items 的既有 `evidenceRefs`；`evidenceMessages` 只供 Reducer 校验这些既有证据的 quote，不能被 Proposer 用来摘录新事实。
...
- 固定范围以完整 segment 为单位：一个 section/path 一旦被纳入本 Proposer 输入，就全量输入该 segment 的当前 items，不做 last N、相似度筛选或按 `blockedPatchSummary` 语义筛选。
...
普通 Proposer 的 read-only context：
...
| `task.targetSections` / `task.targetPaths` | 被预算阻塞的 section/path | `targetPaths` 对 `core` 必填；禁止跨 section 或跨 core path 合并 |
...
3. compaction task 由同一 `userId/presetId` 串行队列执行，输入使用同一 envelope：`task.mode` 为 `"maintenance"`，`task.trigger` 为 `{ type: "lengthBudget", limit, blockedPatchSummary }`；`writableState` 只包含目标 section/path 的既有 items 全集且保留 `evidenceRefs` 和 `evidenceKind`，不做数量截断或 `blockedPatchSummary` 语义筛选；`evidenceMessages` 包含 source item 既有证据对应的 raw messages；`readOnlyContext` 按 `compactionProposer` 的固定范围补充背景防止误合并。
...
`compactionProposer` 的输入不是普通最近对话窗口，而是 `mode: "maintenance"` 的同形 envelope。它仍然可以读取固定背景，但证据只能来自 `writableState` 中 source items 的既有证据。示例：
