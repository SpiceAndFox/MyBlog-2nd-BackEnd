...
2. 按 Proposer family 聚合 eligible targets，生成 `eligibleTasks`。
...
- Observer 将 eligible targets 聚合成 `eligibleTasks`，每个 task 对应一个 Proposer。
...
3. compaction task 与普通 task 共用同一 `userId/presetId` 串行队列（见 §2），使用 [state-contract.md](state-contract.md) §5.2 的 maintenance 模式 envelope。其 `targetKey` 只关联来源 normal target，不表示 compaction 拥有或推进 raw-message cursor。
