...
2. **Proposer**（按记忆族调用，schema-constrained structured output）：每个专用 Proposer 只处理自己负责的 section，输出 patch / noop / unable_to_decide，并按 op 附 evidenceKind 和 evidenceRefs。
...
2. **messageId 存在性校验**：普通模式非 `mergeItems` patch 的 `evidenceRefs.messageId` 是否在 `observedMessages` 范围内且真实存在；`mergeItems` 不接收 Proposer 输出的 `evidenceRefs`，Reducer 校验 `itemIds` 指向的 source items 存在，并从 source items 继承既有 `evidenceRefs`。
...
- `contextWindow`（M ≥ N）：发给 Proposer 的观察窗口大小。`observedMessageIds = 该 user/preset 下最近 M 条消息的 id 列表（按 id ASC，取最后 M 条）`。窗口内 id > cursor 的消息是本轮新消息，id <= cursor 的是重叠上下文。普通写入 patch 的 evidenceRefs 可以引用 observedMessages 中的任意消息（含重叠部分），但 patch 应反映新信息。
...
5. Reducer 对 compaction patch 继续执行 schema、itemIds、policy、结构化冲突和 source evidence 完整性校验。`memory_compaction` 的 evidenceRefs 由 Reducer 根据 `itemIds` 从 source items 继承。
