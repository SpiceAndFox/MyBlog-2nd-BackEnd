...
1. **schema 校验**：patch 的 op、path/itemId、value 是否符合 [state-contract.md](state-contract.md) §4 的 Patch Op 约束；若 `task.targetPaths` 非空，patch 的 `path` 必须在其中。`todos` 的 `addItem`/`updateItem` 若 `value` 含 `expiresAt`，解析结构化表达式并计算 `expiresAtTime`（见 [state-contract.md](state-contract.md) §4），计算结果在过去则 reject（reason: `invalid_expiry_in_past`）。
...
compaction task 有界执行：同一 section/path 对同一阻塞窗口最多尝试 1 次。resume 重置该有界执行计数，允许重新尝试同一阻塞窗口。
...
`deferred` 不推进：该 section 等待 compaction task 有界执行；compaction 成功后重新跑同一窗口，compaction 无合并空间或合并后仍超限时触发 halt。
