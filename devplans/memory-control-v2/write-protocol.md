...
1. **Observer**（纯代码）：读取最近对话、当前 state、各 section cursor，按 lag 阈值计算 eligible sections，组装结构化输入。
2. **Proposer**（单次 LLM 调用，function calling）：看到 eligible sections 的消息后，自行决定每个 section 输出 patch 还是 noop，并附 evidenceKind 和 evidenceRefs。
3. **Reducer**（纯代码）：schema 校验 → messageId 存在性 → quote 模糊匹配 → policy gate（按 evidenceKind 查表）→ 结构化冲突检测 → apply。
4. **Renderer**（纯代码模板）：把最新 state 渲染为主聊天模型可读的 memory 文本，写入 `state_v2_render`。

职责边界：

- **Observer 只算 lag，不做信号检测**。是否需要更新某 section 由 Proposer 看到消息后自行判断——Proposer 本来就要理解消息，让它自路由比预先用规则/LLM 检测信号更可靠。
- **Proposer 只提出候选 patch + 自评 evidenceKind**，不判断最终可信度，不输出自由置信度分数。
...
Observer 的职责是构造一次 memory tick 的结构化输入。它只做两件事：

1. 对每个 section 计算 `lag = recent window 末端 message id - 该 section 的 coveredUntilMessageId`，lag 超过阈值的 section 为 eligible。
2. 为每个 eligible section 收集 observedMessageIds（lag 范围内的消息），组装结构化 JSON 输入。
...
Proposer 单次 LLM 调用，**必须使用 function calling / structured output**（tool use）强制输出 schema，不能靠裸 prompt + 文本解析。这是保证 JSON 可靠性的关键手段。

Proposer 看到 eligible sections 的消息后，自行决定每个 section 输出什么：
...
如果 Proposer 没输出某个 eligible section 的结果，Reducer 视为 `error`，不猜测推进。
...
- 只有 eligible section 出现在 Proposer 输入和输出契约中。
- 非 eligible section 不出现在 Proposer 输出的 `sectionResults` 中，其 cursor 不推进。
- **哪些 section 实际发生变化，由 Proposer 看到 messages 后自行决定**（输出 patch 或 noop），不由 Observer 预判。
...
| `milestones`     | 10                 | 里程碑低频，但 lag 不能太大    |
...
- `error`：重试 1 次。仍失败则记录 error event，推进 cursor（避免永久卡死），该 section 使用上一次稳定 render。

连续多次 error 的 section：记录告警日志，保留旧 render，其它 section 继续推进。不需要独立的"冻结"状态——cursor 推进后下次 tick 自然重新评估。
...
- `recentEpisodes` 按窗口自然滚出（Reducer 清理超出上限的旧 item）；只有真正关键的 episode 由 Proposer 主动输出 `addItem` 到 `milestones`。
- `todos` 只能因完成、取消、失效或澄清而删除（`completeTodo`/`cancelTodo`/`expireTodo`/`correctItem`）。
- `milestones` 默认不删除，只允许 `mergeItems` 和基于 `user_correction` 的 `correctItem`。
...
旧 `rolling_summary` 和 `core_memory` 不直接转换为 v2 state。需要迁移旧会话时，从原始 `chat_messages` 回放：对旧消息按批次跑 v2 pipeline（Observer → Proposer → Reducer → Renderer），生成 v2 state。
...
- **Proposer LLM 调用失败**（网络/超时/safety policy）：记录 `error`，重试 1 次。仍失败则推进 cursor，保留旧 render，记录告警日志。
...
- **Section 连续多次 error**：记录告警日志，保留旧 render，其它 section 继续推进。
- **全局连续失败**（所有 section 连续 error）：记录告警日志，保留旧 render。完全停止 memory 写入只能由人工或明确开关触发。
...
Proposer 输出（通过 function calling 返回，schema 强制）：
...
`core` section 的 patch 额外需要 `path` 指定子数组：
