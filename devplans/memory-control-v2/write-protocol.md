...
2. 读取 `chat_memory_target_status`：只有 status 允许 normal 调度的 target 才形成 eligible intent；`retry_wait` 等到 `nextRetryAt`，`halted/rebuilding` 不形成普通 proposal。每个 normal target 唯一映射到一个专用 Proposer。
...
2. Reducer 不立即最终拒绝，也不推进该 target cursor，而是记录 `decision: "deferred"`（[state-contract.md](state-contract.md) §9.2，`deferred` 不填 `reject_reason`），关联 `maintenance_task_id`，并创建一个 compaction task。
...
6. 如果 compaction 成功释放容量，原 section 保持 cursor 不变，在下一次 tick 重新处理同一消息窗口（`deferred` 已阻止 cursor 推进，无论同 tick 是否存在 `accepted` patch，见 §3）。如果 compactionProposer 返回 `unable_to_compact`（无安全合并空间），halt 对应 target。如果 compaction `accepted` 但合并后仍超限，原 capacity-blocked patch 最终 `rejected: length_budget_exceeded`，halt 对应 target。如果 compaction 发生技术性失败（LLM 调用/schema/provider），按 §3.1 error 恢复策略处理。
...
| `rejected`（`length_budget_exceeded`） | 不推进      | events  | compaction 无合并空间或合并后仍超限；halt 对应 target（见 §3.1）                                                                              |
| `deferred`                             | 不推进      | events  | item patch 被长度预算阻塞，已触发 compaction task                                                                                              |
...
**混合 accepted+deferred 时的 reprocess 语义**：当同一 target 同一 tick 既有 `accepted` 又有 `deferred` patch 时，cursor 不推进（`deferred` 阻止推进）。compaction 成功后，target 重新处理同一消息窗口——Proposer 看到更新后的 `writableState`（已 `accepted` 的 patch 已在 state 中），对已处理内容输出 `noop`，对原 `deferred` patch 重新输出。compaction 返回 `unable_to_compact` 或合并后仍超限时，halt 对应 target（见 §3.1）。本段的原子提交/revision 细节由第 5 批状态机替换。

`deferred` 不推进：该 target 等待 compaction task 有界执行；compaction 成功后重新跑同一窗口，compaction 无合并空间或合并后仍超限时 halt 对应 target。
...
resume 只更新指定 per-target status：重置为 healthy、清错误计数/nextRetryAt，并将可恢复 task 重新置为 queued；不改 `memory_state`，不产生 revision/snapshot。worker 随后从 durable task 或该 target cursor 继续，不跳过消息。服务器维护脚本可以操作 halted target，普通 proposal 不得绕过 halt。
...
- 不推进 cursor，记 events（decision=`deferred`）。触发 compaction task。compaction 成功释放容量 → 下一 tick 重跑同一消息窗口；compaction 返回 `unable_to_compact` 或合并后仍超限 → halt 对应 target；compaction 技术性失败 → 按 error 策略处理。

**unable_to_compact（compactionProposer 判定无安全合并空间）**

- tick orchestrator 在运行状态事务中写 ops log、将 maintenance task 置为 failed，并将对应 target status 置为 halted。不增加 revision/snapshot。
- resume 后只重置该 target status 与对应 task 的有界执行计数，重新尝试。

**length_budget_exceeded（compaction accepted 但合并后仍超限）**

- Reducer 记录 `rejected: length_budget_exceeded`，同事务终结 task 并将对应 target status 置为 halted；是否形成 revision 取决于该 task bundle 是否同时提交了 state/cursor 变化。
- resume 后只重置该 target status 与对应 task 的有界执行计数，重新尝试。
