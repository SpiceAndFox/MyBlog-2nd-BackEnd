...
7. scene 字段 patch 的 `capacity_exceeded` rejection 不改变 target status；独立诊断投影把它映射为 active `scene_capacity_exceeded`。Renderer 标记该类记忆可能滞后，健康告警说明“最近一次更新因长度超限未写入”；只有被拒字段后续 accepted 后才清除相应 pending path，全部 pending paths 恢复后才关闭告警。
