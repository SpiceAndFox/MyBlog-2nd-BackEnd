...
{
  "op": "mergeItems",
  "itemIds": ["userProfile:1", "userProfile:3"],
  "value": { "text": "偏好: 不喜欢被连续追问 | 讨厌突然肢体接触" },
  "evidenceKind": "user_correction"
}
```

普通模式下 profileRelationshipProposer 在同一 writable section 中发现重叠 item 时主动合并。不输出 evidenceRefs；Reducer 从 source items 继承 evidenceGroups。
