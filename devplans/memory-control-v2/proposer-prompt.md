...
3. patch 必须附 evidenceKind；除 mergeItems 外，patch 必须附 evidenceRefs。evidenceRefs 的 quote 必须是 observedMessages 中的原始消息短片段（<=80字），不要改写。
4. 普通写入 patch 的 evidenceRefs 必须来自 observedMessages；readOnlyContext 只能用于理解背景，不能作为证据，也不能被当作完整世界状态来推断缺失事实。
...
6. 不输出 evidenceRefs；Reducer 会根据 itemIds 从 source items 继承证据。
...
`evidenceKind` 只能是 `memory_compaction`。Reducer 根据 itemIds 从 source items 继承 evidenceRefs。
...
```
value.text 是 source items 的高密度合并，不引入新事实。evidenceRefs 由 Reducer 继承。
