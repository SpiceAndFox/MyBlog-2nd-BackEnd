...
2. 下一 attempt 读取该字段并发送扩大的 contextWindow；首版固定为原 `contextWindow` 的 2 倍，保持同一 `cursorBefore/targetMessageId/taskId`，只向前扩展 overlap，不纳入 captured target boundary 之后的消息。
