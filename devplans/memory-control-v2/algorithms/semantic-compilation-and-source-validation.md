...
DayOfMonth：读取相同 direct anchor；在用户时区中选择 anchor 当天或之后最近一次有效的目标日号，再转换为该日期结束后的首个日界线。月中已错过目标日号时进入下一个包含该日号的月份，不对 2 月 30/31 日做截断。

support-only change 出现 relative/dayOfMonth date、anchor 非 direct source 或 anchor metadata 不一致时返回 `date_anchor_invalid`。不得使用 task.now/worker/Provider time 猜测。
...
- relative/dayOfMonth direct anchor、support-only anchored date reject、月末/无效月日/DST；
