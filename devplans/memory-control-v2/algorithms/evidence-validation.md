...
4. direct candidate 的 role/createdAt 与 artifact `messageMeta` 一致；support candidate 的持久化 provenance 只有 `messageId + contentHash`，不虚构或比较不存在的 role/createdAt 快照，其 createdAt 以当前权威数据库行为准；
