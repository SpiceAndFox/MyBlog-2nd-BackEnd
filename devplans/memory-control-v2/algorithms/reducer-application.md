...
2. **evidence source 与 new-batch 校验**：普通模式非 `mergeItems` patch 的 `evidenceRefs.messageId` 必须在 `observedMessages` 范围内且真实存在；数据库中的 user/preset/role/createdAt/contentHash 必须与 proposal-time task payload 一致，并按 evidenceKind 校验真实 role。每个 patch 至少一条 evidenceRef 必须位于 `cursorBefore < messageId <= targetMessageId`，否则以 `overlap_only_evidence` 拒绝；overlap evidence 仍可作为其余 refs 消解指代。profile/relationship 的 `factBasis=observedPattern` 还必须有至少 3 个不同 messageId；证据是否覆盖至少两个独立互动片段由 Proposer 负责语义判断。`mergeItems` 不接收 Proposer 输出的 `evidenceRefs`，Reducer 校验 `itemIds` 指向的 source items 存在且带有结构合法的 `evidenceGroups`。完整算法见 [Evidence 校验与 Quote 匹配](evidence-validation.md)。
...
5. **结构化冲突与精确重复检测**：检查同字段覆盖（`setField`）、同 itemId 操作（`updateItem`/`forgetItem`/`completeTodo` 等）、跨 section 合并、itemId 是否真实存在和操作顺序合法性；`addItem`/`updateItem` 还执行 Unicode 规范化 text 完全相等与非 multi-value profile canonicalKey 唯一检查。完全相等不是文本相似度或语义匹配，Reducer 仍不做开放式语义冲突检测。
