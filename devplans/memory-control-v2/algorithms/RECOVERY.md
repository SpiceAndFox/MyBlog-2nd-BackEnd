...
stderr 使用简短的单行提示，每行带 `[PID 进程号]`，用于区分新旧命令的输出。启动时先显示 `[已启动]`，运行中区分：
...
Ctrl+C 停止后续调度和输出修复重试，显示 `[正在停止]`；已发出的请求结束或达到自身超时后，关闭数据库连接。正常取消不打印异常堆栈或整轮任务列表，也不再输出等待/进度提示。数据库关闭失败等真实异常仍会显示。迁移命令指定的报告文件继续保存，`completedTaskCount` 按嵌套结果中实际完成的任务去重统计。

进程真正退出时才显示 `[PID 进程号] [已退出 130]`；正常完成为 `[已退出 0]`。在 Windows 下通过 npm 启动时，PowerShell 提示符可能早于实际 rebuild Node 进程退出而返回，**必须等旧 PID 的“已退出”消息后再重跑**。本次改动不强制杀死正在提交的进程，也不增加跨进程互斥锁。已保存的任务、退避时间和 checkpoint 保留，重新执行原命令即可在原进度上获得新的预算。强制终止或崩溃可能无法输出正常退出消息。
