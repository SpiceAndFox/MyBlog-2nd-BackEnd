...
stderr 使用简短的单行提示，不显示 PID、启动或退出通知。运行中区分：
...
Ctrl+C 或 SIGTERM 会立即终止 CLI 进程（退出码 130），不等待 provider 请求，也不输出停止提示、退出提示、异常堆栈或任务列表。随后可直接重新执行原命令，无需等待额外通知。终止时不再生成新的迁移完成报告。

已提交的任务、退避时间、cursor 和 checkpoint 保留；连接断开时尚未提交的数据库事务回滚，正在提交的事务以数据库最终状态为准。重跑获得新的内存预算并从持久化状态恢复。如果另一条执行已提交同一任务，迟到的结果会识别已完成状态，重建重新读取当前进度；真正的游标冲突和 halt 仍会阻止提交。正常完成和其他真实错误保留原有报告及资源清理，`completedTaskCount` 按嵌套结果中实际完成的任务去重统计。
