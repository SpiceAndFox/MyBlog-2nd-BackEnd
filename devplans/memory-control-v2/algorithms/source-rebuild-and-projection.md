...
v1 清理与 v2 启服是两个独立动作。确认 v1 worker 和上下文注入已经停用后，可以显式执行 `clearLegacyDerivedMemory`：

```text
确认 v1 runtime 已停用
→ 明确确认清除 v1 派生数据
→ 清空 rolling/core Memory 字段并删除 v1 checkpoint
→ 校验 legacy residue 为 0
```

该动作必须幂等，不触碰 `chat_messages`、v2 state/event/snapshot/task 或 RAG/Recall 数据。提前执行意味着不再支持回退到 v1 摘要系统，但不会自动启用 v2，也不能绕过 v2 的 rebuild/校验门。
