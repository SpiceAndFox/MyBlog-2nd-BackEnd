...
本文是 source mutation、sourceGeneration、rebuild、force drain 以及 RAG projection drain 的单一权威来源。Recall/Scene Recall 是继承 RAG cutoff 的查询时 enrichment。State、task、target status 和 projection checkpoint 的静态 shape 见 [状态契约](../state-contract.md) §1、§9。

## 1. Source Generation

`memory_state.meta.sourceGeneration` 是 Memory 与 RAG 共享的 raw-source 世代，查询时 Recall 继承 RAG 对该世代的覆盖。普通追加 User/Assistant message 不增加 generation，只唤醒 normal worker。
...
RAG invalidation 不依赖通用 outbox，并持久化唯一的派生 projection checkpoint，至少包含 `processedGeneration`、`processedBoundaryMessageId` 与 `processedTombstoneId`；不得从 Memory target cursor 推定 projection 处理进度。Recall/Scene Recall 不创建空操作 checkpoint，其覆盖和 tombstone gate 直接继承 RAG 查询边界。
...
生产环境的具体命令、备份点、停启服证明、报告归档和失败处置需等 migration CLI 与真实 RAG projection adapter 完成装配后定稿，当前登记在 [Memory v2 生产切换执行手册（Deferred）](../../deferred/memory-v2-production-migration-runbook.md)，不得用尚未验证的临时命令代替正式 runbook。
