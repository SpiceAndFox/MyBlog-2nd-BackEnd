...
上下文装配新增单一 `memoryV2` segment：当 v2 feature flag 开启且 `memory_state` 可用时，`memoryV2` 读取结构化状态并调用 Renderer 实时生成完整 v2 memory 文本。旧 `rollingSummary` / `coreMemory` segment 禁用，避免重复注入。
...
| C16  | Renderer 接入     | 新增单一 `memoryV2` segment；读取时实时 render，v2 可用时禁用旧 core/rolling segment       |
