...
- [渲染与上下文接入](memory-control-v2/rendering-and-context.md)：Renderer、`memory` segment、RAG 边界、raw message 输入边界。
...
LLM 只负责观察对话并提出结构化 patch。不同记忆族使用不同 Proposer：`currentStateProposer` 处理 `scene` / `participants`，`todoProposer` 处理 `todos`，`episodeProposer` 处理 `recentEpisodes` / `milestones`，`coreProposer` 处理长期核心档案。长度预算造成的维护任务由独立 `compactionProposer` 处理，它只能合并既有 item，不能新增事实或静默删除长期记忆。最终写入必须经过确定性 Reducer：schema 校验、messageId 存在性、quote 模糊匹配、policy gate、结构化冲突检测和长度预算。Reducer 不做开放式自然语言理解，也不直接调用 LLM。
...
Memory 写入统一使用 raw message evidence：user 与 assistant 消息都从原始 `chat_messages` 读取。assistant gist 不进入 v2 memory proposer 输入，也不作为 evidenceRef 来源；gist的用途为避免assistant的语言风格随着对话轮数增加逐渐趋同，而非压缩上下文。
...
| C7   | Evidence 输入     | user 与 assistant 统一使用 raw message；assistant gist 不作为 memory evidence    |
