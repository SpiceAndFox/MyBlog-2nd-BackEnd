...
- **用第二个 LLM 调用去判第一个 LLM 的输出成本翻倍但可靠性提升存疑**：首版不采用。验证靠纯代码 + Proposer 自评的 evidenceKind。
...
LLM 只负责观察对话并提出结构化 patch。不同记忆族使用不同 Proposer：`currentStateProposer` 处理 `scene` / `participants`，`todoProposer` 处理 `todos`，`episodeProposer` 处理 `recentEpisodes` / `milestones`，`coreProposer` 处理长期核心档案。最终写入必须经过确定性 Reducer：schema 校验、messageId 存在性、quote 模糊匹配、policy gate、结构化冲突检测和长度预算。Reducer 不做开放式自然语言理解，也不直接调用 LLM。
...
| C5   | LLM 调用          | 按记忆族调用专用 Proposer（function calling）；无独立 Verifier LLM               |
...
| C13  | Proposer 输出契约 | function calling 强制 schema；每个目标 section 输出 `patches                     | noop | unable_to_decide` |
...
Memory Control v2 的本质是：用专用 LLM Proposer 观察历史并提出结构化 patch，用 function calling 保证输出可靠，用纯代码 Reducer 做确定性校验和写入，用事件日志保证可审计，用 Renderer 在读取时给主模型提供稳定上下文。
