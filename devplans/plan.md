...
7. **可审计**：patch 决策（accepted / rejected / deferred / error）写入事件表，供调试和排查。
...
| C3   | 事件审计          | accepted / rejected / deferred / error 写入 `chat_memory_events`      | state-contract §9       |
...
| C6   | Core 写入         | 内容新增/改写只接受 `long_term_fact` 或 `user_correction`；维护合并只允许重叠项 | write-protocol §4       |
...
| C10  | 失败兜底          | 保留稳定 state，不回退到 v1 全文摘要重写                              | write-protocol §8       |
