...
LLM 只负责观察对话并提出结构化 patch。不同记忆族使用不同 Proposer：`currentStateProposer` 处理 `scene` / `participants`，`todoProposer` 处理 `todos`，`episodeProposer` 处理 `recentEpisodes` / `milestones`，`coreProposer` 处理长期核心档案。Proposer 输入使用 envelope：`writableState` 限制写入范围，`readOnlyContext` 提供跨 section 背景，`evidenceMessages` 提供可校验原文证据。长度预算造成的维护任务由独立 `compactionProposer` 处理，它只能合并既有 item，不能新增事实或静默删除长期记忆。最终写入必须经过确定性 Reducer：schema 校验、messageId 存在性、quote 模糊匹配、policy gate、结构化冲突检测和长度预算。Reducer 不做开放式自然语言理解，也不直接调用 LLM。
