...
- [状态契约](memory-control-v2/state-contract.md)：`memory_state`、section、item、evidenceKind、patch op、长度预算。
- [写入协议](memory-control-v2/write-protocol.md)：Observer、专用 Proposer、Reducer、路由、cursor、quote 校验、policy gate、事件表。
- [渲染与上下文接入](memory-control-v2/rendering-and-context.md)：Renderer、`memory` segment、RAG 边界、Proposer 输入与 raw evidence 边界。
...
- **LLM 不能稳定输出复杂嵌套 JSON**：必须用 provider 支持的 schema-constrained structured output 强制 schema；可以是 function/tool calling，也可以是 JSON schema response format，不能靠裸 prompt + 文本解析。
...
- **用第二个 LLM 调用去判第一个 LLM 的输出成本翻倍但可靠性提升存疑**：首版不采用。验证靠纯代码 + Proposer 输出的 evidenceKind 枚举分类。
...
LLM 只负责观察对话并提出结构化 patch。不同记忆族使用不同 Proposer：`currentStateProposer` 处理 `scene` / `participants`，`todoProposer` 处理 `todos`，`episodeProposer` 处理 `recentEpisodes` / `milestones`，`coreProposer` 处理长期核心档案。Proposer 输入使用 envelope：`writableState` 限制写入范围，`readOnlyContext` 按 Proposer 类型提供固定跨 section 背景，纳入的 section/path 以完整 segment 输入，`evidenceMessages` 提供可校验原文证据。长度预算造成的维护任务由独立 `compactionProposer` 处理，它只能合并既有 item，不能新增事实或静默删除长期记忆。最终写入必须经过确定性 Reducer：schema 校验、messageId 存在性、quote 模糊匹配、policy gate、结构化冲突检测和长度预算。Reducer 不做开放式自然语言理解，也不直接调用 LLM。
...
上下文装配使用单一 `memory` segment：当 `memory_state` 存在且 schema 校验通过时，`memory` 读取结构化状态并调用 Renderer 实时生成完整 memory 文本。若 `memory_state` 不存在或版本不支持，该 segment 直接不注入，并在日志中报错（Memory超出一定轮数时才生成、注入，遵循原逻辑）。

RAG 不替代 memory control。RAG 负责从历史中找相关原文片段，Memory Control 负责维护当前稳定状态和长期档案；两者在 context compiler 层并列注入。

Memory 写入统一使用 raw message evidence：user 与 assistant 消息都从原始 `chat_messages` 读取。Proposer 可以读取由 `memory_state` 裁剪出的 read-only context 来理解当前对话，但普通写入 patch 的 evidenceRef 只能来自 `evidenceMessages`；read-only context 不能直接证明新事实。assistant gist 不进入 v2 memory proposer 输入，也不作为 evidenceRef 来源；gist的用途为避免assistant的语言风格随着对话轮数增加逐渐趋同，而非压缩上下文。
...
固定 prompt 统一管理是独立重构计划：后续所有长期维护的固定 prompt 都应迁移到 `prompts/<domain>/*` 下，由 service 读取；service 文件只保留动态变量拼装、参数传递和调用逻辑。运行时临时拼接的短模板可以留在代码里，但系统/任务级 prompt 不应散落在 service 实现中。
...
| 编号 | 决策              | 结果                                                                             |
| ---- | ----------------- | -------------------------------------------------------------------------------- |
| C1   | 权威状态          | `memory_state` JSONB 是唯一权威 memory state                                     |
| C2   | 写入权            | Proposer 产出 patch proposal + evidenceKind 枚举分类；纯代码 Reducer 决定最终写入 |
| C3   | 事件审计          | accepted / rejected / deferred / error 写入 `chat_memory_events`（精简表）       |
| C4   | Section 推进      | 每个 section 独立 cursor，同一 `userId/presetId` 单队列串行                      |
| C5   | LLM 调用          | 按记忆族调用专用 Proposer（schema-constrained output）；无独立 Verifier LLM       |
| C6   | Core 写入         | 内容新增/改写只接受 `long_term_fact` 或 `user_correction`；维护合并只允许重叠项 |
| C7   | Evidence 输入     | raw message 是唯一写入证据；read-only context 只作背景，assistant gist 不作证据  |
| C8   | RAG 边界          | RAG 召回具体历史，memory 保存持续状态                                            |
| C9   | 迁移              | 旧文本不直接转 v2，旧会话迁移必须基于原始消息回放                                |
| C10  | 失败兜底          | 保留稳定 state，不回退到 v1 全文摘要重写                                         |
| C11  | 存储落点          | `chat_preset_memory` 新增 `memory_state` JSONB；不新增权威 render 列             |
| C12  | eligible tasks    | per-section / per-family lag 阈值决定本轮调度哪些 Proposer                       |
| C13  | Proposer 输出契约 | structured output 强制 schema；每个目标 section 输出 patches/noop/unable_to_decide |
| C14  | Reducer 职责      | 纯代码：schema/ID/quote 模糊匹配/policy 查表/结构化冲突；不做 NLU、不做语义匹配  |
| C15  | 结构化输出        | 必须用 schema-constrained structured output，不能靠裸 prompt 解析                |
| C16  | Renderer 接入     | 使用单一 `memory` segment；读取时实时 render，状态不存在或 schema 不支持则不注入 |
| C17  | 部署假设          | 首版单实例，进程内队列串行；不做并发控制                                         |
| C18  | 预算维护          | 长度预算阻塞先 deferred 并触发 compactionProposer；维护失败后才最终拒绝          |
...
Memory Control v2 的本质是：用专用 LLM Proposer 观察历史并提出结构化 patch，用 schema-constrained structured output 保证输出可靠，用纯代码 Reducer 做确定性校验和写入，用事件日志保证可审计，用 Renderer 在读取时给主模型提供稳定上下文。
