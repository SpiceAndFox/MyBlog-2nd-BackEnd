...
| C17  | 预算维护          | 长度预算阻塞时 capacity-blocked 并创建 maintenance task；compaction 后确定性 replay 原 proposal；compaction/replay 失败时 halt 对应 target（临时方案，待容量默认值稳定后引入自动降级，见 [容量降级策略（延后）](deferred/memory-control-v2/capacity-degradation.md)） | [Compaction 与 Proposal Replay 算法](memory-control-v2/algorithms/compaction-and-replay.md) |
