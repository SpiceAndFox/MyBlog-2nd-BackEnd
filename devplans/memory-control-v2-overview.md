...
- 不在本轮实现 LLM Suppression Proposer、Gap Compressor，或长期 overdue todo 的精细归档/检索/清理；延后项统一见 [延后设计清单](memory-control-v2-deferred/readme.md)。
...
| C10  | 失败兜底          | 保留稳定 state；连续错误只 halt 对应 target，手动 resume 指定 target；不回退到 v1 全文摘要重写 | write-protocol §3.1, §8 |
...
| C17  | 预算维护          | 长度预算阻塞时 capacity-blocked 并创建 maintenance task；compaction 后确定性 replay 原 proposal；compaction/replay 失败时 halt 对应 target | write-protocol §2.1     |
| C18  | 健康与告警        | 用户侧只暴露 healthy/degraded/rebuilding；非健康原因持续可见，halt 只作用于对应 target，主聊天继续 | write-protocol §8       |
| C19  | Source rebuild    | sourceGeneration 是 Memory/RAG/Recall 共享世代；source mutation 原子进入 rebuilding，并用既有 tasks force drain | write-protocol §7       |
...
| C22  | Privacy hard delete | 跨 raw、Memory 历史/任务 payload、RAG/Recall 与 debug 派生存储物理清除，并从剩余 source rebuild 后才恢复服务 | write-protocol §5.4     |
...
| C24  | 串行与幂等        | 单实例 per-user/preset 串行；stable task/dedupe identity 与 generation/cursor/revision 提交校验保证重复恢复不重复 apply | state-contract §9.2-§9.3, §9.6 |
