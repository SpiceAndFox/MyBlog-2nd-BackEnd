...
Reducer 的核心校验链（schema → messageId → quote → policy → 冲突 → 预算 → lifecycle → apply → 事件）不变。Snapshot 写入是 memory 事务提交后的副作用，不影响校验决策。`scene_expired`、`expired_scene_evicted`、`todo_became_overdue`、`recent_episode_evicted` 仍以 Memory v2 的 system cleanup event/revision/snapshot 为权威；本 Recall 派生表不得改变这些状态。
