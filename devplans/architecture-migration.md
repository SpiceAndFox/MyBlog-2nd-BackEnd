...
状态：已完成（含 2026-07-22 边界收口与 2026-07-23 Chat LLM owner 归位）
...
完成审计发现，早期阶段记录曾把“模块直接依赖根 `config`/`db`/`logger`/`services/llm`”、Chat Repository 读取 Memory 表，以及进程级 Memory facade 记为可接受过渡状态；这与本计划第 3 节规则 1、4、5、8、9 及第 6 节完成标准直接冲突。它们只能作为未完成项，不能与“迁移完成”同时成立。补充收口批次已消除这些依赖，并把相应规则固化为自动门禁。
...
RAG 暂作为 Chat 子模块；出现多个独立消费者后再评估提升为顶层模块。Chat completion、Provider catalog、model/settings schema 和协议 adapter 归入 `modules/chat/infrastructure/llm`，embedding/reranker 归入 `modules/chat/rag/infrastructure`。Memory 保持独立的严格 structured-output Provider infrastructure。只有稳定且确实被多个模块复用的 transport、SSE 或协议适配能力才考虑迁入 `shared/llm`。
...
- Chat application 定义自己需要的 LLM 调用端口；实现最终归入 Chat owner，不建立跨 Chat/Memory 的高层 LLM service。
...
本批次不扩大业务范围，只修正 A-F 完成声明与实际边界之间的差距：

- Chat 发送 use case 通过共享 transaction executor，在同一 PostgreSQL transaction/client 内调用 Memory source-write guard 与 Chat Repository；Chat Repository 不再查询 Memory-owned 表；
- Memory source mutation、rebuild、privacy delete 与 Chat user/assistant 写入使用同一 scope advisory transaction lock，generation 与 privacy fence 在锁内读取；
- Memory Repository、Chat RAG、Auth 日志与时区能力全部改为显式工厂参数注入；仅 `app/composition` 连接根配置、数据库和日志；
- 删除 `services/chat/memoryRuntime.js` 进程级可变 facade，runtime 实例只由 composition 持有；
- 架构门禁新增“业务模块只能依赖自身或 `shared`”和“SQL 不得越过表 owner”两类检查。

实施记录见 [E 阶段记录](./architecture-migration-phase-e.md) 与 [F 阶段记录](./architecture-migration-phase-f.md) 的完成审计修订。

### H. Chat LLM owner 归位（补充批次）

- 删除 `services/llm` 的全部受版本控制实现，不保留兼容入口；
- Provider registry、OpenRouter attribution、completion adapter、RAG embedding/reranker 和 production model policy 全部改为实例工厂，不保存已配置环境或根配置单例；
- 根 `config` 通过参数接收 Chat LLM catalog 与 Memory config loader，不再反向导入业务模块；
- Chat/RAG 由 composition 接收同一个 Chat LLM runtime；Memory 继续使用自己的 strict structured-output transport；
- `shared/llm` 仍不建立，等待真实的跨 owner 纯协议复用。

实施记录：[H 阶段 Chat LLM owner 归位](./architecture-migration-phase-h.md)。
