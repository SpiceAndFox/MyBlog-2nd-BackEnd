...
2. 当前 Renderer artifact 的 `publicInput` 作为 user message；
3. strict Semantic JSON Schema 或强制 tool call 约束输出；
4. 本地 Semantic Validator、Compiler 与 Reducer 依次执行 ref/source/date 编译和 state/policy 复核。
...
2. 单一示例容易造成主题、措辞和 change 数量锚定；多样示例又会持续增加每次调用的输入 token。
...
- 示例覆盖正例、noop、unable、update 和高风险误提取边界，不能只展示 `add`；
- 示例主题与线上评测集分离，且人物、地点、物品、措辞多样；
- 示例输出必须通过当前 Provider Semantic Schema 和本地 Semantic Validator；
...
- profile 持久信息与一次性动作区分、主体归属及单次过度归纳；
- old-only direct、support-only、同义重复、correction、forget、unable_to_decide；
- 不同主题、关系阶段、角色语气和消息长度；
- DeepSeek strict-tools 与其他已支持 transport。

指标至少包括：

- Semantic change 级 precision/recall，且将错误长期记忆的 false positive 设为高权重；
...
在这些条件满足前，不修改当前 `system + artifact.publicInput user message` 的生产调用结构。
