...
> 已确认版本：Memory Control `"2.01"`。版本作为字符串持久化，不使用浮点数。2.01 不兼容旧 v2 派生 state/task/event/snapshot，开发数据库直接重建。
...
- `action`：领域动作；
- `section`：由外层 `sectionResults` key 唯一确定，change 内不重复保存；联合 Proposer 仍必须逐 section 给出 `changes/noop/unable_to_decide` 终局；
...
Todo relative date 必须显式提供 `anchorMessageId`，且它必须属于同 change 的直接 `evidenceMessageIds`。Compiler 使用该消息的数据库 `createdAt` 和 task 固化的用户时区规范化；support-only change 不得产生 relative date。
...
所有 item section 都允许 `add/update/correct/forget`；scene 允许 `set/correct/clear/forget`，其中 scene forget 等价于 clear。Todo 与 Agreement 继续保留各自 terminal action。
...
不新增 `evidenceEffect`。`correct` 直接作为 Semantic action；它与 `update` 编译到相同的持久化更新操作，并且当前不保留不同的 event/诊断语义。
...
- 持久化 provenance 固定为扁平 `sourceRefs: [{ messageId, contentHash }]`；
- 当前 state、compiled Patch 和 event 不保存 quote；如未来诊断确有需求，另行设计受控、可按 scope 删除的 debug 数据，而不恢复 LLM quote 协议。
...
后续方案已登记在 `devplans/deferred/memory-control-v2/correction-forget-suppression.md`，覆盖完整的 correction / forget tombstone、RAG / Recall filtering、rebuild 终态过滤和可选片段级 Suppression Proposer。2.01 主设计文档必须同步删除旧 tombstone correctness 约束，不能让主设计与本延期决策并存。
...
确定性编译错误不推进 cursor、不产生 revision/snapshot/event，不自动盲重试；若先发现 generation/revision/cursor 已变化，则优先走既有 stale/successor 规则。否则 task failed、对应 target halted，并写 `ref_resolution_failed`、`source_validation_failed`、`date_anchor_invalid` 或 `compile_invariant_failed`。
